...
class OnlineController extends Model {
	 ...
	 * @return void
	 */
	async addOnline(string ip, int customer_id, string url, string referer): void {
		this->db->query("DELETE FROM `" . DB_PREFIX . "customer_online` WHERE `date_added` < '" . date('Y-m-d H:i:s', new Date('-' . (int)this->config->get('config_customer_online_expire') . ' hour')) . "'");
