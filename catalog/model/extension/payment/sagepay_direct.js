module.exports = class ModelExtensionPaymentSagePayDirect extends Model {
	...
	async getCard(card_id, token) {
		qry = await this.db.query("SELECT * FROM " + DB_PREFIX + "sagepay_direct_card WHERE (card_id = '" + this.db.escape(card_id) + "' OR token = '" + this.db.escape(token) + "') AND customer_id = '" + await this.customer.getId() + "'");
	...
	async recurringPayment(item, vendor_tx_code) {
		...
		order_recurring_id = await this.model_checkout_recurring.addRecurring(this.session.data['order_id'], recurring_description, item['recurring']);
