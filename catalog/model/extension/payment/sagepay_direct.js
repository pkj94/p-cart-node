...
module.exports = class ModelExtensionPaymentSagePayDirect extends Model {
	async getMethod(address, total) {
		...
		const query = await this.db.query("SELECT * FROM " + DB_PREFIX + "zone_to_geo_zone WHERE geo_zone_id = '" + this.config.get('payment_sagepay_direct_geo_zone_id') + "' AND country_id = '" + address['country_id'] + "' AND (zone_id = '" + address['zone_id'] + "' OR zone_id = '0')");
		let status = false;
		if (Number(this.config.get('sagepay_direct_total')) > 0 && Number(this.config.get('payment_sagepay_direct_total')) > total) {
			status = false;
		} else if (!this.config.get('payment_sagepay_direct_geo_zone_id')) {
			status = true;
	...
	async getCards(customer_id) {
		...
		const query = await this.db.query("SELECT * FROM " + DB_PREFIX + "sagepay_direct_card WHERE customer_id = '" + customer_id + "' ORDER BY card_id");
	...
	async addCard(card_data) {
		await this.db.query("INSERT into " + DB_PREFIX + "sagepay_direct_card SET customer_id = '" + this.db.escape(card_data['customer_id']) + "', digits = '" + this.db.escape(card_data['Last4Digits']) + "', expiry = '" + this.db.escape(card_data['ExpiryDate']) + "', type = '" + this.db.escape(card_data['CardType']) + "', token = '" + this.db.escape(card_data['Token']) + "'");

		return this.db.getLastId();
	}

	async updateCard(card_id, token) {
		await this.db.query("UPDATE " + DB_PREFIX + "sagepay_direct_card SET token = '" + this.db.escape(token) + "' WHERE card_id = '" + card_id + "'");
	}

	async getCard(card_id, token) {
		const qry = await this.db.query("SELECT * FROM " + DB_PREFIX + "sagepay_direct_card WHERE (card_id = '" + this.db.escape(card_id) + "' OR token = '" + this.db.escape(token) + "') AND customer_id = '" + await this.customer.getId() + "'");
	...
	async updateOrder(order_info, data) {
		await this.db.query("UPDATE `" + DB_PREFIX + "sagepay_direct_order` SET `SecurityKey` = '" + this.db.escape(data['SecurityKey']) + "',  `VPSTxId` = '" + this.db.escape(data['VPSTxId']) + "', `TxAuthNo` = '" + this.db.escape(data['TxAuthNo']) + "' WHERE `order_id` = '" + order_info['order_id'] + "'");

		return this.db.getLastId();
	}

	async deleteOrder(vendor_tx_code) {
		await this.db.query("DELETE FROM `" + DB_PREFIX + "sagepay_direct_order` WHERE order_id = '" + vendor_tx_code + "'");
	}

	async addTransaction(sagepay_direct_order_id, type, order_info) {
		await this.db.query("INSERT INTO `" + DB_PREFIX + "sagepay_direct_order_transaction` SET `sagepay_direct_order_id` = '" + sagepay_direct_order_id + "', `date_added` = now(), `type` = '" + this.db.escape(type) + "', `amount` = '" + this.currency.format(order_info['total'], order_info['currency_code'], false, false) + "'");
	}
	...
	async recurringPayment(item, vendor_tx_code) {
		...
		this.load.model('extension/payment/sagepay_direct', this);
		//trial information
		...
		await this.model_checkout_recurring.addReference(order_recurring_id, vendor_tx_code);

		const order_info = await this.model_checkout_order.getOrder(this.session.data['order_id']);

		const sagepay_order_info = await this.getOrder(this.session.data['order_id']);

		const response_data = await this.setPaymentData(order_info, sagepay_order_info, price, order_recurring_id, item['recurring']['name']);
		...
		} else if (item['recurring_trial'] == 1) {
			next_payment = await this.calculateSchedule(item['recurring']['trial_frequency'], next_payment, item['recurring']['trial_cycle']);
		...
		await this.addRecurringOrder(this.session.data['order_id'], response_data, order_recurring_id, date('Y-m-d H:i:s', trial_end), date('Y-m-d H:i:s', subscription_end));
	...
	async cronPayment() {
		...
		const log = new Log('sagepay_direct_recurring_orders.log');
	...
	async updateCronJobRunTime() {
		await this.db.query("DELETE FROM `" + DB_PREFIX + "setting` WHERE `code` = 'sagepay_direct' AND `key` = 'payment_sagepay_direct_last_cron_job_run'");
		await this.db.query("INSERT INTO `" + DB_PREFIX + "setting` (`store_id`, `code`, `key`, `value`, `serialized`) VALUES (0, 'sagepay_direct', 'payment_sagepay_direct_last_cron_job_run', NOW(), 0)");
	}
