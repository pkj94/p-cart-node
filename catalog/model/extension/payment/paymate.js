module.exports = class ModelExtensionPaymentPayMate extends Model {
	async getMethod(address, total) {
		...
		let status = true;
		if (Number(this.config.get('payment_paymate_total')) > 0 && Number(this.config.get('payment_paymate_total')) > total) {
			status = false;
		...
		if (!currencies.includes(this.session.data['currency'].toUpperCase())) {
			status = false;
		}
