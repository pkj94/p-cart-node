...
class ModelExtensionPaymentWorldpay extends Model {
	...
	async getTransactions(worldpay_order_id) {
		qry = await this.db.query("SELECT * FROM `" + DB_PREFIX + "worldpay_order_transaction` WHERE `worldpay_order_id` = '" + worldpay_order_id + "'");
	...
	async cronPayment() {
		...
		for (profiles as profile) {
			...
			order_info = await this.model_checkout_order.getOrder(profile['order_id']);
	...
	async getRecurringOrder(order_recurring_id) {
		qry = await this.db.query("SELECT * FROM " + DB_PREFIX + "worldpay_order_recurring WHERE order_recurring_id = '" + order_recurring_id + "'");
		return qry.row;
	}
	...
	async getWorldpayOrder(worldpay_order_id) {
		qry = await this.db.query("SELECT * FROM " + DB_PREFIX + "worldpay_order WHERE order_code = " + worldpay_order_id);
		return qry.row;
	}
