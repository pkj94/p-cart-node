module.exports = class ModelExtensionPaymentPilibaba extends Model {
	...
	async getOrderTaxAmount(order_id) {
		const query = await this.db.query("SELECT SUM(`value`) AS `value` FROM `" + DB_PREFIX + "order_total` WHERE `order_id` = '" + order_id + "' AND `code` = 'tax'");
	...
	async addPilibabaOrder(response_data) {
		await this.db.query("INSERT INTO `" + DB_PREFIX + "pilibaba_order` SET `order_id` = '" + response_data['orderNo'] + "', `amount` = '" + response_data['orderAmount'] + "', `fee` = '" + response_data['fee'] + "', `tracking` = '', `date_added` = NOW()");
	}
