module.exports = class ModelExtensionPaymentFirstdataRemote extends Model {
	...
	async capturePayment(data, order_id) {
		...
		const order_ref = 'API-' + order_id + '-' + date("Y-m-d-H-i-s") + '-' + Math.random(10, 500);

		const amount = this.currency.format(order_info['total'], order_info['currency_code'], order_info['currency_value'], false);
		let type = 'preAuth';
		...
		let token = '';
		let payment_token = '';
		if (Number(this.config.get('firstdata_remote_card_storage')) == 1) {
			if ((this.request.post['cc_choice']) && this.request.post['cc_choice'] != 'new') {
				payment_token = this.request.post['cc_choice'];
			} else if ((this.request.post['cc_store']) && this.request.post['cc_store'] == 1) {
				token = sha1(await this.customer.getId() + '-' + date("Y-m-d-H-i-s") + Math.random(10, 500));
			}
		}
		...
		response['payment_type'] = (string[0]) ? string[0] : '';

		string = xml.xpath('//ipgapi:Brand');
		response['brand'] = (string[0]) ? string[0] : '';

		string = xml.xpath('//ipgapi:Country');
		response['country'] = (string[0]) ? string[0] : '';

		string = xml.xpath('//ipgapi:ProcessorReceiptNumber');
		response['receipt_number'] = (string[0]) ? string[0] : '';

		string = xml.xpath('//ipgapi:ProcessorTraceNumber');
		response['trace_number'] = (string[0]) ? string[0] : '';

		string = xml.xpath('//ipgapi:ProcessorCCVResponse');
		response['ccv'] = (string[0]) ? string[0] : '';

		string = xml.xpath('//ipgapi:AVSResponse');
		response['avs'] = (string[0]) ? string[0] : '';

		response['card_number_ref'] = substr(data['cc_number'], -4);

		if (strtoupper(response['transaction_result']) == 'APPROVED' && (token)) {
			this.storeCard(token, this.customer.getId(), response['brand'], data['cc_expire_date_month'], data['cc_expire_date_year'], substr(data['cc_number'], -4));
		}
