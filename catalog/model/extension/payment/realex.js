module.exports = class ModelExtensionPaymentRealex extends Model {
	...
	async addTransaction(realex_order_id, type, order_info) {
		await this.db.query("INSERT INTO `" + DB_PREFIX + "realex_order_transaction` SET `realex_order_id` = '" + realex_order_id + "', `date_added` = now(), `type` = '" + this.db.escape(type) + "', `amount` = '" + this.currency.format(order_info['total'], order_info['currency_code'], order_info['currency_value'], false) + "'");
	}
