...
module.exports = class ModelExtensionPaymentRealexRemote extends Model {
	async getMethod(address, total) {
		...
		let status = false;
		if (Number(this.config.get('payment_realex_remote_total')) > 0 && Number(this.config.get('payment_realex_remote_total')) > total) {
			status = false;
		} else if (!this.config.get('payment_realex_remote_geo_zone_id')) {
			status = true;
	...
	async checkEnrollment(account, amount, currency, order_ref) {
		...
		let tmp = timestamp + '.' + merchant_id + '.' + order_ref + '.' + amount + '.' + currency + '.' + this.request.post['cc_number'];
		let hash = sha1(tmp);
		tmp = hash + '.' + secret;
		hash = sha1(tmp);
		...
		await this.logger(await parseStringPromise(xml));
		await this.logger(xml);

		const url = "https://epage.payandshop.com/epage-3dsecure.cgi";
		try {
			const response = await axios.post(url, xml, {
				headers: {
					'User-Agent': `OpenCart ${VERSION || 'unknown'}`, // Set the User-Agent header 
					'Content-Type': 'application/xml' // Assuming the content type is XML 
			...
			await this.logger('Error during HTTP request:', error.message);
			// throw error;
			return error;
		}
	}
	...
	async enrollmentSignature(account, amount, currency, order_ref, card_number, card_expire, card_type, card_name, pares) {
		...
		let tmp = timestamp + '.' + merchant_id + '.' + order_ref + '.' + amount + '.' + currency + '.' + card_number;
		let hash = sha1(tmp);
		tmp = hash + '.' + secret;
		hash = sha1(tmp);
		...
		await this.logger(await parseStringPromise(xml));
		await this.logger(xml);
		let url = "https://epage.payandshop.com/epage-3dsecure.cgi";
		try {
			const response = await axios.post(url, xml, {
				headers: {
					'User-Agent': `OpenCart ${VERSION || 'unknown'}`, // Set the User-Agent header 
	...
	async capturePayment(account, amount, currency, order_id, order_ref, card_number, expire, name, type, cvv, issue, eci_ref, eci = '', cavv = '', xid = '') {
		...
		if (Number(this.config.get('payment_realex_remote_auto_settle')) == 0) {
			xml += '<autosettle flag="0" />';
		} else if (Number(this.config.get('payment_realex_remote_auto_settle')) == 1) {
			xml += '<autosettle flag="1" />';
		} else if (Number(this.config.get('payment_realex_remote_auto_settle')) == 2) {
		...
		if (Number(this.config.get('payment_realex_remote_tss_check')) == 1) {
			xml += '<tssinfo>';
			...
			if (((order_info['payment_iso_code_2']) && (order_info['payment_iso_code_2'])) || ((order_info['payment_postcode']) && (order_info['payment_postcode']))) {
				...
				if (((order_info['payment_postcode']) && (order_info['payment_postcode']))) {
					xml += '<code>' + Number(order_info['payment_postcode']) + '|' + Number(order_info['payment_address_1']) + '</code>';
				}
			...
			if (((order_info['shipping_iso_code_2']) && (order_info['shipping_iso_code_2'])) || ((order_info['shipping_postcode']) && (order_info['shipping_postcode']))) {
				...
				if (((order_info['shipping_postcode']) && (order_info['shipping_postcode']))) {
					xml += '<code>' + Number(order_info['shipping_postcode']) + '|' + Number(order_info['shipping_address_1']) + '</code>';
				}
		...
		let url = "https://epage.payandshop.com/epage-remote.cgi";
		try {
			let response = await axios.post(url, xml, {
				headers: {
					'User-Agent': `OpenCart ${VERSION || 'unknown'}`, // Set the User-Agent header 
					'Content-Type': 'application/xml' // Assuming the content type is XML 
				}, httpsAgent: new (require('https').Agent)({
			...
			let message = '<strong>' + this.language.get('text_result') + ':</strong> ' + response.result;
			...
			if (response.result == '00') {
				...
				const realex_order_id = await this.addOrder(order_info, response, account, order_ref);

				if (this.config.get('payment_realex_remote_auto_settle') == 1) {
					await this.addTransaction(realex_order_id, 'payment', order_info);
					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_realex_remote_order_status_success_settled_id'), message);
				} else {
					await this.addTransaction(realex_order_id, 'auth', 0);
					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_realex_remote_order_status_success_unsettled_id'), message);
				}
			} else if (response.result == "101") {
				// Decline
				await this.addHistory(order_id, this.config.get('payment_realex_remote_order_status_decline_id'), message);
			} else if (response.result == "102") {
				// Referal B
				await this.addHistory(order_id, this.config.get('payment_realex_remote_order_status_decline_pending_id'), message);
			} else if (response.result == "103") {
				// Referal A
				await this.addHistory(order_id, this.config.get('payment_realex_remote_order_status_decline_stolen_id'), message);
			} else if (response.result == "200") {
				// Error Connecting to Bank
				await this.addHistory(order_id, this.config.get('payment_realex_remote_order_status_decline_bank_id'), message);
	...
	async addOrder(order_info, response, account, order_ref) {
		let settle_status = 0;
		if (Number(this.config.get('payment_realex_remote_auto_settle')) == 1) {
			settle_status = 1;
		}

		await this.db.query("INSERT INTO `" + DB_PREFIX + "realex_remote_order` SET `order_id` = '" + order_info['order_id'] + "', `settle_type` = '" + this.config.get('payment_realex_remote_auto_settle') + "', `order_ref` = '" + this.db.escape(order_ref) + "', `order_ref_previous` = '" + this.db.escape(order_ref) + "', `date_added` = now(), `date_modified` = now(), `capture_status` = '" + settle_status + "', `currency_code` = '" + this.db.escape(order_info['currency_code']) + "', `pasref` = '" + this.db.escape(response.pasref) + "', `pasref_previous` = '" + this.db.escape(response.pasref) + "', `authcode` = '" + this.db.escape(response.authcode) + "', `account` = '" + this.db.escape(account) + "', `total` = '" + this.currency.format(order_info['total'], order_info['currency_code'], order_info['currency_value'], false) + "'");

		return this.db.getLastId();
	}

	async addTransaction(realex_remote_order_id, type, order_info) {
		await this.db.query("INSERT INTO `" + DB_PREFIX + "realex_remote_order_transaction` SET `realex_remote_order_id` = '" + realex_remote_order_id + "', `date_added` = now(), `type` = '" + this.db.escape(type) + "', `amount` = '" + this.currency.format(order_info['total'], order_info['currency_code'], order_info['currency_value'], false) + "'");
	}
