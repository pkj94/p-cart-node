module.exports = class ModelExtensionPaymentAlipay extends Model {
	...
	function pagePay(builder,config) {
		...
		biz_content=null;
		if((builder)){
			biz_content = JSON.stringify(builder,JSON_UNESCAPED_UNICODE);
		}
...
//			preString=this.getSignContentUrlencode(totalParams);
			preString=this.getSignContent(encodeURIComponent(totalParams));
			requestUrl = this.gateway_url."?".preString;

			return requestUrl;
		} else {
			for (totalParams as key  value) {
