...
class ModelExtensionPaymentSquareup extends Model {
    ...
    async getMethod(address, total) {
        geo_zone_query = await this.db.query("SELECT * FROM " + DB_PREFIX + "zone_to_geo_zone WHERE geo_zone_id = '" + this.config.get('payment_squareup_geo_zone_id') + "' AND country_id = '" + address['country_id'] + "' AND (zone_id = '" + address['zone_id'] + "' OR zone_id = '0')");
        ...
        minimum_total = this.config.get('payment_squareup_total');
    ...
    async cronEmail(result) {
        ...
        if ((result['transaction_fail'])) {
            ...
            for (result['transaction_fail'] as order_recurring_id  amount) {
        ...
        if ((result['transaction_success'])) {
            ...
            for (result['transaction_success'] as order_recurring_id  amount) {
    ...
    async nextRecurringPayments() {
        ...
        for (await this.db.query(recurring_sql).rows as recurring) {
            ...
            transaction_tenders = @JSON.parse(recurring['tenders'], true);

            price = (recurring['trial'] ? recurring['trial_price'] : recurring['recurring_price']);
    ...
    async updateRecurringExpired(order_recurring_id) {
        ...
        if (recurring_info['trial']) {
            ...
            expirable = recurring_info['recurring_duration'];
        }
        ...
        if (expirable) {
            ...
            total_duration = recurring_info['trial_duration'] + recurring_info['recurring_duration'];
            
            // If successful payments exceed total_duration
            if (number_of_successful_payments >= total_duration) {
                await this.db.query("UPDATE `" + DB_PREFIX + "order_recurring` SET status='" + self::RECURRING_EXPIRED + "' WHERE order_recurring_id='" + order_recurring_id + "'");
    ...
    async suspendRecurringProfile(order_recurring_id) {
        await this.db.query("UPDATE `" + DB_PREFIX + "order_recurring` SET status='" + self::RECURRING_SUSPENDED + "' WHERE order_recurring_id='" + order_recurring_id + "'");

        return true;
    }

    async getLastSuccessfulRecurringPaymentDate(order_recurring_id) {
        return await this.db.query("SELECT date_added FROM `" + DB_PREFIX + "order_recurring_transaction` WHERE order_recurring_id='" + order_recurring_id + "' AND type='" + self::TRANSACTION_PAYMENT + "' ORDER BY date_added DESC LIMIT 0,1").row['date_added'];
    }

    async getRecurring(order_recurring_id) {
        recurring_sql = "SELECT * FROM `" + DB_PREFIX + "order_recurring` WHERE order_recurring_id='" + order_recurring_id + "'";

        return await this.db.query(recurring_sql).row;
    }

    async getTotalSuccessfulPayments(order_recurring_id) {
        return await this.db.query("SELECT COUNT(*) as total FROM `" + DB_PREFIX + "order_recurring_transaction` WHERE order_recurring_id='" + order_recurring_id + "' AND type='" + self::TRANSACTION_PAYMENT + "'").row['total'];
    }
