...
class OrderController extends Model {
	...
	async getOrder(order_id) {
		order_query = await this.db.query("SELECT * FROM `" + DB_PREFIX + "order` WHERE `order_id` = '" + order_id + "' AND `customer_id` = '" + await this.customer.getId() + "' AND `customer_id` != '0' AND `order_status_id` > '0'");

		if (order_query.num_rows) {
			country_query = await this.db.query("SELECT * FROM `" + DB_PREFIX + "country` WHERE `country_id` = '" + order_query.row['payment_country_id'] + "'");
			...
			zone_query = await this.db.query("SELECT * FROM `" + DB_PREFIX + "zone` WHERE `zone_id` = '" + order_query.row['payment_zone_id'] + "'");
