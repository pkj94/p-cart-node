module.exports = class ControllerMailAffiliate extends Controller {
	async index(&route, &args, &output) {
		...
		data['login'] = await this.url.link('affiliate/login', '', true);
		...
		mail.setText(await this.load.view('mail/affiliate', data));
		await mail.send();
 	}
	...
	async alert(&route, &args, &output) {
		...
		if (in_array('affiliate', this.config.get('config_mail_alert'))) {
			...
			mail.setFrom(this.config.get('config_email'));
			mail.setSender(html_entity_decode(this.config.get('config_name')));			
			mail.setSubject(html_entity_decode(this.language.get('text_new_affiliate')));
			mail.setText(await this.load.view('mail/affiliate_alert', data));
			await mail.send();
