module.exports = class ControllerMailOrder extends Controller {
	...
	async add(order_info, order_status_id, comment, notify) {
		...
		order_products = await this.model_checkout_order.getOrderProducts(order_info['order_id']);
		
		for (order_products of order_product) {
		...
		data['link'] = order_info['store_url'] + 'index.js?route=account/order/info&order_id=' + order_info['order_id'];

		if (download_status) {
			data['download'] = order_info['store_url'] + 'index.js?route=account/download';
		} else {
			data['download'] = '';
		}
		...
		for (order_products of order_product) {
			...
			order_options = await this.model_checkout_order.getOrderOptions(order_info['order_id'], order_product['order_product_id']);

			for (order_options of order_option) {
		...
		order_vouchers = await this.model_checkout_order.getOrderVouchers(order_info['order_id']);

		for (order_vouchers of order_voucher) {
		...
		order_totals = await this.model_checkout_order.getOrderTotals(order_info['order_id']);
		...
		from = await this.model_setting_setting.getSettingValue('config_email', order_info['store_id']);
		...
		mail.setSubject(html_entity_decode(sprintf(language.get('text_subject'), order_info['store_name'], order_info['order_id'])));
		mail.setHtml(await this.load.view('mail/order_add', data));
		await mail.send();
	}
	
	async edit(order_info, order_status_id, comment) {
		language = new Language(order_info['language_code']);
		language.load(order_info['language_code']);
		...
		mail.setSender(html_entity_decode(order_info['store_name']));
		mail.setSubject(html_entity_decode(sprintf(language.get('text_subject'), order_info['store_name'], order_info['order_id'])));
		mail.setText(await this.load.view('mail/order_edit', data));
		await mail.send();
	}
	...
	async alert(&route, &args) {
		...
		if (order_info && !order_info['order_status_id'] && order_status_id && in_array('order', this.config.get('config_mail_alert'))) {	
			...
			order_products = await this.model_checkout_order.getOrderProducts(order_id);
			...
			order_vouchers = await this.model_checkout_order.getOrderVouchers(order_id);
			...
			order_totals = await this.model_checkout_order.getOrderTotals(order_id);
			...
			mail.setSender(html_entity_decode(order_info['store_name']));
			mail.setSubject(html_entity_decode(sprintf(this.language.get('text_subject'), this.config.get('config_name'), order_info['order_id'])));
			mail.setText(await this.load.view('mail/order_alert', data));
			await mail.send();
