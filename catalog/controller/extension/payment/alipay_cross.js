...
const data = {};
		...
		alipay_config = array (
			...
			'return_url'           : await this.url.link('checkout/success'),
		...
		if ((this.session.data['currency']) && this.session.data['currency'] == 'CNY') {
		...
		data['params'] = await this.model_extension_payment_alipay_cross.buildRequestPara(alipay_config, parameter);
		...
		return await this.load.view('extension/payment/alipay_cross', data);
	}
	...
	async callback() {
		...
		verify_result = await this.model_extension_payment_alipay_cross.verifyNotify(alipay_config);
		...
		if(verify_result) {//check successed
			...
			if(_POST['trade_status'] == 'TRADE_FINISHED') {
				this.load.model('checkout/order',this);
				await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_alipay_cross_order_status_id'));
