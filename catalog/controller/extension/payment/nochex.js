module.exports = // Nochex via form will work for both simple "Seller" account and "Merchant" account holders
...
const data = {};
		...
		data['success_url'] = await this.url.link('checkout/success', '', true);
		data['cancel_url'] = await this.url.link('checkout/payment', '', true);
		data['declined_url'] = await this.url.link('extension/payment/nochex/callback', 'method=decline', true);
		data['callback_url'] = await this.url.link('extension/payment/nochex/callback', 'order=' + this.session.data['order_id'], true);

		return await this.load.view('extension/payment/nochex', data);
	}

	async callback() {
		await this.load.language('extension/payment/nochex');

		if ((this.request.get['method']) && this.request.get['method'] == 'decline') {
		...
		for (this.request.post of key : value) {
			request += '&' + key + '=' + encodeURIComponent(stripslashes(value));
		}
		...
		if (strcmp(response, 'AUTHORISED') == 0) {
			await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_nochex_order_status_id'));
		} else {
			await this.model_checkout_order.addOrderHistory(order_id, this.config.get('config_order_status_id'), 'Auto-Verification step failed+ Manually check the transaction+');
		}
