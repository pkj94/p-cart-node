...
const data = {};
		...
		if (this.config.get('firstdata_remote_card_storage') == 1 && await this.customer.isLogged()) {
			data['card_storage'] = 1;
			data['stored_cards'] = await this.model_extension_payment_firstdata_remote.getStoredCards();
		} else {
			data['card_storage'] = 0;
		...
		return await this.load.view('extension/payment/firstdata_remote', data);
	}
	...
	async send() {
		...
		if (!(this.request.post['cc_choice']) || ((this.request.post['cc_choice']) && this.request.post['cc_choice'] == 'new')) {
		...
		if (empty(json['error'])) {
			...
			capture_result = await this.model_extension_payment_firstdata_remote.capturePayment(this.request.post, order_id);

			message = '';

			if ((capture_result['transaction_result']) && strtoupper(capture_result['transaction_result']) == 'APPROVED') {
				...
				fd_order_id = await this.model_extension_payment_firstdata_remote.addOrder(order_info, capture_result);

				if (this.config.get('firstdata_remote_auto_settle') == 1) {
					await this.model_extension_payment_firstdata_remote.addTransaction(fd_order_id, 'payment', order_info);

					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('firstdata_remote_order_status_success_settled_id'), message, false);
				} else {
					await this.model_extension_payment_firstdata_remote.addTransaction(fd_order_id, 'auth');

					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('firstdata_remote_order_status_success_unsettled_id'), message, false);
				}
			} else {
				if ((capture_result['error']) && !empty(capture_result['error'])) {
					json['error'] = capture_result['error'];
				} else {
					json['error'] = this.language.get('error_failed');
				}

				if ((capture_result['fault']) && !empty(capture_result['fault'])) {
				...
				await this.model_extension_payment_firstdata_remote.addHistory(order_id, this.config.get('firstdata_remote_order_status_decline_id'), message);
			}
		}
