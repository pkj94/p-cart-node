...
const data = {};
		if (this.config.get('payment_opayo_vendor')) {
			...
			if (data['logged'] && data['card_save']) {
				...
				data['cards'] = await this.model_extension_payment_opayo.getCards(await this.customer.getId());
			}
			...
			for (i = today['year']; i < today['year'] + 11; i++) {
				data['years'].push(array(
					'code' : sprintf('%04d', i),
					'name'  : sprintf('%02d', i % 100)
				});
			}

			return await this.load.view('extension/payment/opayo', data);
		}
	}
	...
	async confirm() {
		...
		order_products = await this.model_account_order.getOrderProducts(this.session.data['order_id']);
		...
		order_totals = await this.model_account_order.getOrderTotals(this.session.data['order_id']);
		...
		payment_data['ClientIPAddress'] = this.request.server.headers['x-forwarded-for'] || (
		...
		payment_data['ThreeDSNotificationURL'] = str_replace('&amp;', '&', await this.url.link('extension/payment/opayo/threeDSnotify', 'order_id=' + this.session.data['order_id'], true));
		...
		response_data = await this.model_extension_payment_opayo.sendCurl(url, payment_data);
		...
		if (response_data['Status'] == '3DAUTH') {
			...
			if (!empty(payment_data['CreateToken']) && await this.customer.isLogged()) {
				card_data = array();
				...
				card_id = await this.model_extension_payment_opayo.addCard(card_data);
			} else if (!empty(payment_data['Token'])) {
				card = await this.model_extension_payment_opayo.getCard(false, payment_data['Token']);
				card_id = card['card_id'];
			}
			
			await this.model_extension_payment_opayo.addOrder(this.session.data['order_id'], response_data, payment_data, card_id);
			
			await this.model_extension_payment_opayo.log('Response Data', response_data);
			await this.model_extension_payment_opayo.log('Payment Data', payment_data);
			await this.model_extension_payment_opayo.log('Order Id', this.session.data['order_id']);

			json['TermUrl'] = str_replace('&amp;', '&', await this.url.link('extension/payment/opayo/threeDSnotify', 'order_id=' + this.session.data['order_id'], true));
		} else if (response_data['Status'] == 'OK' || response_data['Status'] == 'AUTHENTICATED' || response_data['Status'] == 'REGISTERED') {
			message = '';

			if ((response_data['TxAuthNo'])) {
				message += 'TxAuthNo: ' + response_data['TxAuthNo'] + "\n";
			} else {
				response_data['TxAuthNo'] = '';
			}

			if ((response_data['AVSCV2'])) {
				message += 'AVSCV2: ' + response_data['AVSCV2'] + "\n";
			}

			if ((response_data['AddressResult'])) {
				message += 'AddressResult: ' + response_data['AddressResult'] + "\n";
			}

			if ((response_data['PostCodeResult'])) {
				message += 'PostCodeResult: ' + response_data['PostCodeResult'] + "\n";
			}

			if ((response_data['CV2Result'])) {
				message += 'CV2Result: ' + response_data['CV2Result'] + "\n";
			}

			if ((response_data['3DSecureStatus'])) {
				message += '3DSecureStatus: ' + response_data['3DSecureStatus'] + "\n";
			}

			if ((response_data['CAVV'])) {
				message += 'CAVV: ' + response_data['CAVV'] + "\n";
			}
			
			card_id = '';
			
			if (!empty(payment_data['CreateToken']) && !empty(response_data['Token']) && await this.customer.isLogged()) {
				card_data = array();
			...
			opayo_order_id = await this.model_extension_payment_opayo.addOrder(order_info['order_id'], response_data, payment_data, card_id);
			
			await this.model_extension_payment_opayo.log('Response Data', response_data);
			await this.model_extension_payment_opayo.log('Payment Data', payment_data);
			await this.model_extension_payment_opayo.log('Order Id', this.session.data['order_id']);

			await this.model_extension_payment_opayo.addOrderTransaction(opayo_order_id, setting['general']['transaction_method'], order_info);

			await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], setting['general']['order_status_id'], message, false);
			...
			if (setting['general']['transaction_method'] == 'PAYMENT') {
				...
				for (recurring_products of item) {
					await this.model_extension_payment_opayo.recurringPayment(item, payment_data['VendorTxCode']);
				} 
			}
			...
			await this.model_extension_payment_opayo.log('Response data', json['error']);
		}
	...
	async threeDSnotify() {
		...
		if ((this.request.get['order_id'])) {
			opayo_order_info = await this.model_extension_payment_opayo.getOrder(this.request.get['order_id']);
			...
			response_data = await this.model_extension_payment_opayo.sendCurl(url, this.request.post);
			...
			if (response_data['Status'] == 'OK' || response_data['Status'] == 'AUTHENTICATED' || response_data['Status'] == 'REGISTERED') {
				...
				order_info = await this.model_checkout_order.getOrder(this.request.get['order_id']);
				opayo_order_info = await this.model_extension_payment_opayo.getOrder(this.request.get['order_id']);

				await this.model_extension_payment_opayo.log('Order Info', order_info);
				await this.model_extension_payment_opayo.log('Opayo Order Info', opayo_order_info);

				await this.model_extension_payment_opayo.updateOrder(order_info, response_data);
				await this.model_extension_payment_opayo.addOrderTransaction(opayo_order_info['opayo_order_id'], setting['general']['transaction_method'], order_info);
				await this.model_checkout_order.addOrderHistory(this.request.get['order_id'], setting['general']['order_status_id'], message, false);

				if (!empty(response_data['Token']) && await this.customer.isLogged()) {
					await this.model_extension_payment_opayo.updateCard(opayo_order_info['card_id'], response_data['Token']);
				} else {
					await this.model_extension_payment_opayo.deleteCard(opayo_order_info['card_id']);
				}
				...
				if (setting['general']['transaction_method'] == 'PAYMENT') {
					...
					//loop through any products that are recurring items
					for (recurring_products of item) {
						await this.model_extension_payment_opayo.recurringPayment(item, opayo_order_info['VendorTxCode']);
					}
				}
	...
	async deleteCard() {
		...
		card = await this.model_extension_payment_opayo.getCard(false, this.request.post['opayo_card_token']);
		...
		if (!empty(card['token'])) {
			...
			if (response_data['Status'] == 'OK') {
				await this.model_extension_payment_opayo.deleteCard(card['card_id']);
	...
	async cron() {
		...
		if ((this.request.get['token']) && hash_equals(setting['cron']['token'], this.request.get['token'])) {
			this.load.model('extension/payment/opayo');
	
			orders = await this.model_extension_payment_opayo.cronPayment();

			await this.model_extension_payment_opayo.updateCronRunTime();

			await this.model_extension_payment_opayo.log('Repeat Orders', orders);
		}
	}
