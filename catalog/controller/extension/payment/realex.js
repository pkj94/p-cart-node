...
const data = {};
			...
			for (accounts of card : account) {
				if ((account['enabled']) && account['enabled'] == 1) {
					data['cards'].push(array(
						...
						'account' : ((account['default']) && account['default'] == 1 ? this.config.get('payment_realex_merchant_id') : account['merchant_id']),
					});
				}
			}
		...
		if (this.config.get('payment_realex_auto_settle') == 0) {
			data['settle'] = 0;
		} else if (this.config.get('payment_realex_auto_settle') == 1) {
			data['settle'] = 1;
		} else if (this.config.get('payment_realex_auto_settle') == 2) {
			data['settle'] = 'MULTI';
		}

		data['tss'] = this.config.get('payment_realex_tss_check');
		...
		data['response_url'] = HTTPS_SERVER + 'index.js?route=extension/payment/realex/notify';

		return await this.load.view('extension/payment/realex', data);
	}

	async notify() {
		this.load.model('extension/payment/realex');

		await this.model_extension_payment_realex.logger(print_r(this.request.post, 1));
		...
		if (hash != this.request.post['SHA1HASH']) {
			...
			auto_settle = this.config.get('payment_realex_auto_settle');
			tss = this.config.get('payment_realex_tss_check');
			...
			if (this.request.post['RESULT'] == "00") {
				realex_order_id = await this.model_extension_payment_realex.addOrder(order_info, this.request.post['PASREF'], this.request.post['AUTHCODE'], this.request.post['ACCOUNT'], this.request.post['ORDER_ID']);

				if (auto_settle == 1) {
					await this.model_extension_payment_realex.addTransaction(realex_order_id, 'payment', order_info);
					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_realex_order_status_success_settled_id'), message, false);
				} else {
					await this.model_extension_payment_realex.addTransaction(realex_order_id, 'auth', 0+00);
					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_realex_order_status_success_unsettled_id'), message, false);
				}
				...
				// Decline
				await this.model_extension_payment_realex.addHistory(order_id, this.config.get('payment_realex_order_status_decline_id'), message);
				data['text_response'] = this.language.get('text_decline');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "102") {
				// Referal B
				await this.model_extension_payment_realex.addHistory(order_id, this.config.get('payment_realex_order_status_decline_pending_id'), message);
				data['text_response'] = this.language.get('text_decline');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "103") {
				// Referal A
				await this.model_extension_payment_realex.addHistory(order_id, this.config.get('payment_realex_order_status_decline_stolen_id'), message);
				data['text_response'] = this.language.get('text_decline');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "200") {
				// Error Connecting to Bank
				await this.model_extension_payment_realex.addHistory(order_id, this.config.get('payment_realex_order_status_decline_bank_id'), message);
		...
		this.response.setOutput(await this.load.view('extension/payment/realex_response', data));
	}
}