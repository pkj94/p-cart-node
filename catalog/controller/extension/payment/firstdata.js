...
const data = {};
		...
		data['currency'] = await this.model_extension_payment_firstdata.mapCurrency(order_info['currency_code']);
		...
		data['url_success'] = await this.url.link('checkout/success', '', true);
		data['url_fail'] = await this.url.link('extension/payment/firstdata/fail', '', true);
		data['url_notify'] = await this.url.link('extension/payment/firstdata/notify', '', true);
		...
		if (this.config.get('payment_firstdata_card_storage') == 1 && await this.customer.isLogged()) {
			data['card_storage'] = 1;
			data['stored_cards'] = await this.model_extension_payment_firstdata.getStoredCards();
			data['new_hosted_id'] = sha1(await this.customer.getId()  + '-' + date("Y-m-d-H-i-s") + rand(10, 500));
		} else {
			data['card_storage'] = 0;
			data['stored_cards'] = array();
		}

		return await this.load.view('extension/payment/firstdata', data);
	}
	...
	async notify() {
		...
		if (this.config.get('payment_firstdata_debug') == 1) {
			await this.model_extension_payment_firstdata.logger(print_r(this.request.post, 1));
		}

		if ((this.request.post['txntype']) && (this.request.post['notification_hash']) && (this.request.post['oid'])) {
			local_hash = await this.model_extension_payment_firstdata.responseHash(this.request.post['chargetotal'], this.request.post['currency'], this.request.post['txndatetime'], this.request.post['approval_code']);
			...
			if (local_hash == this.request.post['notification_hash']) {
				...
				if (this.request.post['txntype'] == 'preauth' || this.request.post['txntype'] == 'sale') {
					if ((this.request.post['approval_code'])) {
						...
						if (response_parts[0] == 'Y') {
							if ((response_parts[3])) {
								...
								if ((this.request.post['cardnumber'])) {
									message += this.language.get('text_response_card') + this.request.post['cardnumber'] + '<br />';
								}

								if ((this.request.post['processor_response_code'])) {
									message += this.language.get('text_response_proc_code') + this.request.post['processor_response_code'] + '<br />';
								}

								if ((this.request.post['refnumber'])) {
									message += this.language.get('text_response_ref') + this.request.post['refnumber'] + '<br />';
								}

								if ((this.request.post['paymentMethod'])) {
									message += this.language.get('text_response_card_type') + card_types[strtoupper(this.request.post['paymentMethod'])] + '<br />';
								}
							}

							if ((this.request.post['hosteddataid']) && order_info['customer_id'] != 0) {
								await this.model_extension_payment_firstdata.storeCard(this.request.post['hosteddataid'], order_info['customer_id'], this.request.post['expmonth'], this.request.post['expyear'], this.request.post['cardnumber']);
							}

							fd_order_id = await this.model_extension_payment_firstdata.addOrder(order_info, this.request.post['oid'], this.request.post['tdate']);

							if (this.config.get('payment_firstdata_auto_settle') == 1) {
								await this.model_extension_payment_firstdata.addTransaction(fd_order_id, 'payment', order_info);

								await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_firstdata_order_status_success_settled_id'), message, false);
							} else {
								await this.model_extension_payment_firstdata.addTransaction(fd_order_id, 'auth');

								await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_firstdata_order_status_success_unsettled_id'), message, false);
							}
						} else {
							...
							await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_firstdata_order_status_decline_id'), message);
						}
					}
				}
				...
				if (this.request.post['txntype'] == 'void') {
					if (this.request.post['status'] == 'DECLINED') {
						fd_order = await this.model_extension_payment_firstdata.getOrder(order_id);

						await this.model_extension_payment_firstdata.updateVoidStatus(order_id, 1);

						await this.model_extension_payment_firstdata.addTransaction(fd_order['firstdata_order_id'], 'void');

						await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_firstdata_order_status_void_id'), message, false);
					}
				}
				...
				if (this.request.post['txntype'] == 'postauth') {
					if (this.request.post['status'] == 'APPROVED') {
						...
						await this.model_extension_payment_firstdata.updateCaptureStatus(order_id, 1);

						await this.model_extension_payment_firstdata.addTransaction(fd_order['firstdata_order_id'], 'payment', order_info);

						await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_firstdata_order_status_success_settled_id'), message, false);
					}
				}
			} else {
				await this.model_extension_payment_firstdata.logger('Hash does not match! Received: ' + this.request.post['notification_hash'] + ', calculated: ' + local_hash);
			}
		} else {
			await this.model_extension_payment_firstdata.logger('Data is missing from request + ');
		}
	}
	...
	async fail() {
		...
		if ((this.request.post['fail_reason']) && !empty(this.request.post['fail_reason'])) {
