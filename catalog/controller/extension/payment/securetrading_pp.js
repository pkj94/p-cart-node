...
const data = {};
		...
		if (order_info) {
			...
			payment_country = await this.model_localisation_country.getCountry(order_info['payment_country_id']);
			payment_zone = await this.model_localisation_zone.getZone(order_info['payment_zone_id']);

			shipping_country = await this.model_localisation_country.getCountry(order_info['shipping_country_id']);
			shipping_zone = await this.model_localisation_zone.getZone(order_info['shipping_zone_id']);
			...
			if ((shipping_country['iso_code_3']) && shipping_country['iso_code_3'] == 'USA') {
				data['shipping_county'] = shipping_zone['code'];
			} else {
				data['shipping_county'] = order_info['shipping_zone'];
			}

			if (!(shipping_country['iso_code_2'])) {
			...
			return await this.load.view('extension/payment/securetrading_pp', data);
		}
	}
	...
	async ipn() {
		...
		if (hash_equals(hash('sha256', string_to_hash), this.request.post['responsesitesecurity']) && this.request.post['sitereference'] == this.config.get('payment_securetrading_pp_site_reference')) {
			order_info = await this.model_checkout_order.getOrder(this.request.post['orderreference']);
			...
			if (order_info) {
				...
				if (order_total == this.request.post['mainamount'] && order_info['currency_code'] == this.request.post['currencyiso3a'] && order_info['payment_code'] == 'securetrading_pp') {
					...
					shipping_country = await this.model_extension_payment_securetrading_pp.getCountry(this.request.post['customercountryiso2a']);
					payment_country = await this.model_extension_payment_securetrading_pp.getCountry(this.request.post['billingcountryiso2a']);
					...
					await this.model_extension_payment_securetrading_pp.editOrder(order_info['order_id'], order_info);
					...
					if ((this.request.post['transactionreference'])) {
						transactionreference = this.request.post['transactionreference'];
					} else {
						transactionreference = '';
					}
					await this.model_extension_payment_securetrading_pp.addReference(order_info['order_id'], transactionreference);

					if (this.request.post['errorcode'] == '0') {
						order_status_id = this.config.get('payment_securetrading_pp_order_status_id');

						await this.model_extension_payment_securetrading_pp.confirmOrder(order_info['order_id'], order_status_id);
						await this.model_extension_payment_securetrading_pp.updateOrder(order_info['order_id'], order_status_id, message);
					} else if (this.request.post['errorcode'] == '70000') {
