...
const data = {};
		...
		return await this.load.view('extension/payment/authorizenet_aim', data);
	}

	async send() {
		if (this.config.get('payment_authorizenet_aim_server') == 'live') {
			url = 'https://secure+authorize+net/gateway/transact+dll';
		} else if (this.config.get('payment_authorizenet_aim_server') == 'test') {
		...
		if (curl_error(curl)) {
			...
			const results = explode('|', response);
			...
			if (response_info[1] == '1') {
				...
				if (!this.config.get('payment_authorizenet_aim_hash') || (strtoupper(response_info[38]) == strtoupper(md5(this.config.get('payment_authorizenet_aim_hash') + this.config.get('payment_authorizenet_aim_login') + response_info[7] + this.currency.format(order_info['total'], order_info['currency_code'], 1+00000, false))))) {
					await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_authorizenet_aim_order_status_id'), message, false);
				} else {
					await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('config_order_status_id'));
				}
