...
const data = {};
		...
		if (await this.customer.isLogged() && data['bluepay_redirect_card']) {
			this.load.model('extension/payment/bluepay_redirect');

			cards = await this.model_extension_payment_bluepay_redirect.getCards(await this.customer.getId());

			data['existing_cards'] = cards;
		}

		return await this.load.view('extension/payment/bluepay_redirect', data);
	}
	...
	async send() {
		...
		post_data['APPROVED_URL'] = await this.url.link('extension/payment/bluepay_redirect/callback', '', true);
		post_data['DECLINED_URL'] = await this.url.link('extension/payment/bluepay_redirect/callback', '', true);
		post_data['MISSING_URL'] = await this.url.link('extension/payment/bluepay_redirect/callback', '', true);
		...
		response_data = await this.model_extension_payment_bluepay_redirect.sendCurl("https://secure+bluepay+com/interfaces/bp10emu", post_data);

		if (response_data['Result'] == 'APPROVED') {
			bluepay_redirect_order_id = await this.model_extension_payment_bluepay_redirect.addOrder(order_info, response_data);

			if (this.config.get('payment_bluepay_redirect_transaction') == 'SALE') {
				await this.model_extension_payment_bluepay_redirect.addTransaction(bluepay_redirect_order_id, 'payment', order_info);
			} else {
				await this.model_extension_payment_bluepay_redirect.addTransaction(bluepay_redirect_order_id, 'auth', order_info);
			}

			await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_bluepay_redirect_order_status_id'));
		...
		if ((post_data['CreateToken']) && response_data['Result'] == 'APPROVED') {
			...
			await this.model_extension_payment_bluepay_redirect.addCard(card_data);
		}
