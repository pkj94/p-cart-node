...
const data = {};
		...
		return await this.load.view('extension/payment/realex_remote', data);
	}
	...
	async send() {
		...
		if ((accounts[this.request.post['cc_type']]['default']) && accounts[this.request.post['cc_type']]['default'] == 1) {
		...
		if (this.config.get('payment_realex_remote_3d') == 1) {
			if (this.request.post['cc_type'] == 'visa' || this.request.post['cc_type'] == 'mc' || this.request.post['cc_type'] == 'amex') {
				verify_3ds = await this.model_extension_payment_realex_remote.checkEnrollment(account, amount, currency, order_ref);

				await this.model_extension_payment_realex_remote.logger('Verify 3DS result:\r\n' + print_r(verify_3ds, 1));

				// Proceed to 3D secure
				if ((verify_3ds.result) && verify_3ds.result == '00') {
					enc_data = array(
						'account' : account,
						'amount' : amount,
						'currency' : currency,
					...
					const json = {};
					json['ACSURL'] = verify_3ds.url;
					json['MD'] = md;
					json['PaReq'] = verify_3ds.pareq;
					json['TermUrl'] = await this.url.link('extension/payment/realex_remote/acsReturn', '', true);
				...
				if ((verify_3ds.result) && verify_3ds.result == '110' && (verify_3ds.enrolled) && verify_3ds.enrolled == 'N') {
					eci_ref = 1;
					xid = '';
					cavv = '';
				...
				if ((verify_3ds.result) && verify_3ds.result == '110' && (verify_3ds.enrolled) && verify_3ds.enrolled == 'U') {
				...
				if ((verify_3ds.result)  && verify_3ds.result >= 500 && verify_3ds.result < 600) {
					if (this.config.get('payment_realex_remote_liability') != 1) {
						await this.load.language('extension/payment/realex_remote');

						json['error'] = verify_3ds.message;
		...
		capture_result = await this.model_extension_payment_realex_remote.capturePayment(
			account,
			amount,
			currency,
			order_id,
		...
		await this.model_extension_payment_realex_remote.logger('Capture result:\r\n' + print_r(capture_result, 1));

		if (capture_result.result != '00') {
			json['error'] = capture_result.message + ' (' + capture_result.result + ')';
		} else {
			json['success'] = await this.url.link('checkout/success');
		}
	...
	async acsReturn() {
		if ((this.session.data['order_id'])) {
			...
			md = JSON.parse(this.encryption.decrypt(this.config.get('config_encryption'), post['MD']), true);

			signature_result = await this.model_extension_payment_realex_remote.enrollmentSignature(md['account'], md['amount'], md['currency'], md['order_ref'], md['cc_number'], md['cc_expire'], md['cc_type'], md['cc_name'], post['PaRes']);

			await this.model_extension_payment_realex_remote.logger('Signature result:\r\n' + print_r(signature_result, 1));
			...
			if (signature_result.result == '00' && (strtoupper(signature_result.threedsecure.status) == 'Y' || strtoupper(signature_result.threedsecure.status) == 'A')) {
				...
				eci = signature_result.threedsecure.eci;
				cavv = signature_result.threedsecure.cavv;
				xid = signature_result.threedsecure.xid;
			} else {
				if (md['cc_type'] == 'mc') {
					eci = 0;
				} else {
				...
				if ((signature_result.result)  && signature_result.result >= 500 && signature_result.result < 600) {
					eci_ref = 9;
					xid = '';
					cavv = '';
				}
				...
				if (this.config.get('payment_realex_remote_liability') != 1) {
					...
					message += '<br /><strong>' + this.language.get('text_timestamp') + ':</strong> ' + date("YmdHis");
					message += '<br /><strong>' + this.language.get('text_order_ref') + ':</strong> ' + md['order_ref'];

					if (this.config.get('payment_realex_remote_card_data_status') == 1) {
						message += '<br /><strong>' + this.language.get('entry_cc_type') + ':</strong> ' + md['cc_type'];
						message += '<br /><strong>' + this.language.get('text_last_digits') + ':</strong> ' + substr(md['cc_number'], -4);
						message += '<br /><strong>' + this.language.get('entry_cc_expire_date') + ':</strong> ' + md['cc_expire'];
						message += '<br /><strong>' + this.language.get('entry_cc_name') + ':</strong> ' + md['cc_name'];
					}

					await this.model_extension_payment_realex_remote.addHistory(md['order_id'], this.config.get('payment_realex_remote_order_status_decline_id'), message);
			...
			if (capture_result.result != '00') {
				this.session.data['error'] = capture_result.message + ' (' + capture_result.result + ')';
