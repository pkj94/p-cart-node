...
const data = {};
		...
		return await this.load.view('extension/payment/perpetual_payments', data);
	}
	...
	async send() {
		...
		payment_data = array(
			...
			'cust_ip'       : this.request.server.headers['x-forwarded-for'] || (
		...
		if (response) {
			...
			if ((data[0]) && data[0] == 'A') {
				...
				await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_perpetual_payments_order_status_id'), message, false);
