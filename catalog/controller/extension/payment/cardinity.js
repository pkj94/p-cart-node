...
const data = {};
		...
		return await this.load.view('extension/payment/cardinity', data);
	}
	...
	async send() {
		...
		if (!error) {
			...
			try {
				payment = await this.model_extension_payment_cardinity.createPayment(this.config.get('payment_cardinity_key'), this.config.get('payment_cardinity_secret'), payment_data);
			...
			if (payment) {
				if (!in_array(payment.getStatus(), successful_order_statuses)) {
					...
					await this.model_extension_payment_cardinity.addOrder(array(
					...
					if (payment.getStatus() == 'pending') {
						...
						json['3ds'] = array(
							...
							'TermUrl' : await this.url.link('extension/payment/cardinity/threeDSecureCallback', '', true),
							'hash'    : hash
						});
	...
	async threeDSecureForm() {
		...
		this.response.setOutput(await this.load.view('extension/payment/cardinity_3ds', data));
	}
	...
	async threeDSecureCallback() {
		...
		if (hash_equals(hash, this.request.post['MD'])) {
			order = await this.model_extension_payment_cardinity.getOrder(encryption_data['order_id']);

			if (order && order['payment_id']) {
				payment = await this.model_extension_payment_cardinity.finalizePayment(this.config.get('payment_cardinity_key'), this.config.get('payment_cardinity_secret'), order['payment_id'], this.request.post['PaRes']);
	...
	async finalizeOrder(payment) {
		this.load.model('checkout/order',this);

		await this.load.language('extension/payment/cardinity');

		await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_cardinity_order_status_id'));

		await this.model_extension_payment_cardinity.cardinityLog(this.language.get('text_payment_success'));
		await this.model_extension_payment_cardinity.cardinityLog(payment);
	}

	async failedOrder(log = null, alert = null) {
		await this.load.language('extension/payment/cardinity');

		await this.model_extension_payment_cardinity.cardinityLog(this.language.get('text_payment_failed'));

		if (log) {
			await this.model_extension_payment_cardinity.cardinityLog(log);
		}
	...
	async validate() {
		...
		if (!in_array(order_info['currency_code'], await this.model_extension_payment_cardinity.getSupportedCurrencies())) {
			error['warning'] = this.language.get('error_invalid_currency');
		}

		if (!(this.request.post['holder']) || utf8_strlen(this.request.post['holder']) < 1 || utf8_strlen(this.request.post['holder']) > 32) {
			error['holder'] = true;
		}

		if (!(this.request.post['pan']) || utf8_strlen(this.request.post['pan']) < 1 || utf8_strlen(this.request.post['pan']) > 19) {
			error['pan'] = true;
		}

		if (!(this.request.post['pan']) || !is_numeric(preg_replace('!\s+!', '', this.request.post['pan']))) {
			error['pan'] = true;
		}
		...
		if (!(this.request.post['cvc']) || utf8_strlen(this.request.post['cvc']) < 1 || utf8_strlen(this.request.post['cvc']) > 4) {
			error['cvc'] = true;
		}
