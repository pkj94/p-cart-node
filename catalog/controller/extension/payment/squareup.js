...
const data = {};
        ...
        data['action'] = await this.url.link('extension/payment/squareup/checkout', '', true);
        ...
        if (await this.customer.isLogged()) {
            ...
            cards = await this.model_extension_credit_card_squareup.getCards(await this.customer.getId(), this.config.get('payment_squareup_enable_sandbox'));
        ...
        return await this.load.view('extension/payment/squareup', data);
    }
    ...
    async checkout() {
        ...
        shipping_country_info = await this.model_localisation_country.getCountry(order_info['shipping_country_id']);

        billing_country_info = await this.model_localisation_country.getCountry(order_info['payment_country_id']);
        ...
        try {
            ...
            square_customer = await this.model_extension_credit_card_squareup.getCustomer(await this.customer.getId(), this.config.get('payment_squareup_enable_sandbox'));

            if (!square_customer && await this.customer.isLogged()) {
                square_customer = this.squareup.addLoggedInCustomer();

                await this.model_extension_credit_card_squareup.addCustomer(square_customer);
            }
            ...
            if (await this.customer.isLogged() && !empty(this.request.post['squareup_select_card'])) {
                card_verified = await this.model_extension_credit_card_squareup.verifyCardCustomer(this.request.post['squareup_select_card'],await  this.customer.getId());

                if (!card_verified) {
                    throw new \Squareup\Exception(this.registry, this.language.get('error_card_invalid'));
                }

                card = await this.model_extension_credit_card_squareup.getCard(this.request.post['squareup_select_card']);

                use_saved = true;
                square_card_id = card['token'];
            } else if (await this.customer.isLogged() && (this.request.post['squareup_save_card'])) {
                // Save the card
                card_data = array(
                ...
                if (!await this.model_extension_credit_card_squareup.cardExists(await this.customer.getId(), square_card)) {
                    await this.model_extension_credit_card_squareup.addCard(await this.customer.getId(), this.config.get('payment_squareup_enable_sandbox'), square_card);
                }
            ...
            transaction_data = array(
                ...
                'delay_capture' : !await this.cart.hasRecurringProducts() && this.config.get('payment_squareup_delay_capture'),
            ...
            await this.model_extension_payment_squareup.addTransaction(transaction, this.config.get('payment_squareup_merchant_id'), billing_address, this.session.data['order_id'], user_agent, ip);
            ...
            if (order_status_id) {
                if (await this.cart.hasRecurringProducts() && transaction_status == 'captured') {
                    for (await this.cart.getRecurringProducts() of item) {
                        ...
                        await this.model_extension_payment_squareup.createRecurring(item, this.session.data['order_id'], recurring_description, transaction['id']);
                    }
                }

                order_status_comment = this.language.get('squareup_status_comment_' + transaction_status);

                await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], order_status_id, order_status_comment, true);
            }
            ...
            if (e.isCurlError()) {
                ...
                await this.model_extension_payment_squareup.tokenRevokedEmail();

                json['error'] = this.language.get('text_token_issue_customer_error');
            } else if (e.isAccessTokenExpired()) {
                // Send reminder e-mail to store admin to refresh the token
                await this.model_extension_payment_squareup.tokenExpiredEmail();
