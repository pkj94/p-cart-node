module.exports = class ControllerExtensionPaymentGlobalpayRemote extends Controller {
	async index() {
const data = {};
		...
		return await this.load.view('extension/payment/globalpay_remote', data);
	}
	...
	async send() {
		...
		if (this.config.get('payment_globalpay_remote_3d') == 1) {
			if (this.request.post['cc_type'] == 'visa' || this.request.post['cc_type'] == 'mc' || this.request.post['cc_type'] == 'amex') {
				verify_3ds = await this.model_extension_payment_globalpay_remote.checkEnrollment(account, amount, currency, order_ref);

				await this.model_extension_payment_globalpay_remote.logger('Verify 3DS result:\r\n' + print_r(verify_3ds, 1));
				...
				if ((verify_3ds.result) && verify_3ds.result == '00') {
					...
					json['TermUrl'] = await this.url.link('extension/payment/globalpay_remote/acsReturn', '', true);
		...
		capture_result = await this.model_extension_payment_globalpay_remote.capturePayment(
			account,
			amount,
			currency,
			order_id,
		...
		await this.model_extension_payment_globalpay_remote.logger('Capture result:\r\n' + print_r(capture_result, 1));
	...
	async acsReturn() {
		if ((this.session.data['order_id'])) {
			...
			signature_result = await this.model_extension_payment_globalpay_remote.enrollmentSignature(md['account'], md['amount'], md['currency'], md['order_ref'], md['cc_number'], md['cc_expire'], md['cc_type'], md['cc_name'], post['PaRes']);

			await this.model_extension_payment_globalpay_remote.logger('Signature result:\r\n' + print_r(signature_result, 1));
			...
			if (signature_result.result == '00' && (strtoupper(signature_result.threedsecure.status) == 'Y' || strtoupper(signature_result.threedsecure.status) == 'A')) {
				...
				if (this.config.get('payment_globalpay_remote_liability') != 1) {
					...
					await this.model_extension_payment_globalpay_remote.addHistory(md['order_id'], this.config.get('payment_globalpay_remote_order_status_decline_id'), message);
