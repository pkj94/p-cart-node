...
const data = {};
		...
		response = await this.model_extension_payment_alipay.pagePay(payRequestBuilder,config);
		data['action'] = config['gateway_url'] + "?charset=" + await this.model_extension_payment_alipay.getPostCharset();
		data['form_params'] = response;

		return await this.load.view('extension/payment/alipay', data);
	}
	...
	async callback() {
		...
		result = await this.model_extension_payment_alipay.check(arr, config);
		...
		if(result) {//check successed
			...
			if(_POST['trade_status'] == 'TRADE_FINISHED') {
				...
				this.load.model('checkout/order',this);
				await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_alipay_order_status_id'));
			}
