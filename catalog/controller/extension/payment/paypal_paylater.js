module.exports = class ControllerExtensionPaymentPayPalPayLater extends Controller {
	error = {};
...
const data = {};
		...
		if (this.config.get('payment_paypal_status') && this.config.get('payment_paypal_client_id') && this.config.get('payment_paypal_secret') && agree_status) {
			...
			return await this.load.view('extension/payment/paypal/paypal_paylater', data);
		}
	...
	async modal() {
		...
		this.response.setOutput(await this.load.view('extension/payment/paypal/paypal_paylater_modal', data));
	}
}