...
const data = {};
		...
		if (this.config.get('payment_globalpay_card_select') == 1) {
			...
			for (accounts of card : account) {
				if ((account['enabled']) && account['enabled'] == 1) {
					data['cards'].push(array(
						'type' : card_types[card],
						'account' : ((account['default']) && account['default'] == 1 ? this.config.get('payment_globalpay_merchant_id') : account['merchant_id']),
					});
				}
			}
		...
		if (this.config.get('payment_globalpay_auto_settle') == 0) {
			data['settle'] = 0;
		} else if (this.config.get('payment_globalpay_auto_settle') == 1) {
			data['settle'] = 1;
		} else if (this.config.get('payment_globalpay_auto_settle') == 2) {
			data['settle'] = 'MULTI';
		}

		data['tss'] = this.config.get('payment_globalpay_tss_check');
		...
		data['amount'] = Math.round(this.currency.format(order_info['total'], order_info['currency_code'], order_info['currency_value'], false)*100);
		...
		data['response_url'] = HTTPS_SERVER + 'index.js?route=extension/payment/globalpay/notify';

		return await this.load.view('extension/payment/globalpay', data);
	}

	async notify() {
		this.load.model('extension/payment/globalpay');

		await this.model_extension_payment_globalpay.logger(print_r(this.request.post, 1));
		...
		if (hash != this.request.post['SHA1HASH']) {
			...
			data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
		} else {
			this.load.model('checkout/order',this);
			...
			auto_settle = this.config.get('payment_globalpay_auto_settle');
			tss = this.config.get('payment_globalpay_tss_check');
			...
			if (this.request.post['RESULT'] == "00") {
				globalpay_order_id = await this.model_extension_payment_globalpay.addOrder(order_info, this.request.post['PASREF'], this.request.post['AUTHCODE'], this.request.post['ACCOUNT'], this.request.post['ORDER_ID']);

				if (auto_settle == 1) {
					await this.model_extension_payment_globalpay.addTransaction(globalpay_order_id, 'payment', order_info);
					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('globalpay_order_status_success_settled_id'), message, false);
				} else {
					await this.model_extension_payment_globalpay.addTransaction(globalpay_order_id, 'auth', 0+00);
					await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_globalpay_order_status_success_unsettled_id'), message, false);
				}

				data['text_response'] = this.language.get('text_success');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/success', '', true));
			} else if (this.request.post['RESULT'] == "101") {
				// Decline
				await this.model_extension_payment_globalpay.addHistory(order_id, this.config.get('payment_globalpay_order_status_decline_id'), message);
				data['text_response'] = this.language.get('text_decline');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "102") {
				// Referal B
				await this.model_extension_payment_globalpay.addHistory(order_id, this.config.get('payment_globalpay_order_status_decline_pending_id'), message);
				data['text_response'] = this.language.get('text_decline');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "103") {
				// Referal A
				await this.model_extension_payment_globalpay.addHistory(order_id, this.config.get('payment_globalpay_order_status_decline_stolen_id'), message);
				data['text_response'] = this.language.get('text_decline');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "200") {
				// Error Connecting to Bank
				await this.model_extension_payment_globalpay.addHistory(order_id, this.config.get('payment_globalpay_order_status_decline_bank_id'), message);
				data['text_response'] = this.language.get('text_bank_error');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "204") {
				// Error Connecting to Bank
				await this.model_extension_payment_globalpay.addHistory(order_id, this.config.get('payment_globalpay_order_status_decline_bank_id'), message);
				data['text_response'] = this.language.get('text_bank_error');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
			} else if (this.request.post['RESULT'] == "205") {
				// Comms Error
				await this.model_extension_payment_globalpay.addHistory(order_id, this.config.get('payment_globalpay_order_status_decline_bank_id'), message);
				data['text_response'] = this.language.get('text_bank_error');
				data['text_link'] = sprintf(this.language.get('text_link'), await this.url.link('checkout/checkout', '', true));
		...
		this.response.setOutput(await this.load.view('extension/payment/globalpay_response', data));
	}
}