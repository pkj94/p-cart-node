...
const data = {};
		...
		if (await this.customer.isLogged() && data['sagepay_direct_card']) {
			this.load.model('extension/payment/sagepay_direct');
			data['existing_cards'] = await this.model_extension_payment_sagepay_direct.getCards(await this.customer.getId());
		}

		return await this.load.view('extension/payment/sagepay_direct', data);
	}
...
//			payment_data['VPSProtocol'] = '3+00';
		...
		if (response_data['Status'] == '3DAUTH') {
			...
			if (!empty(payment_data['CreateToken']) && await this.customer.isLogged()) {
				...
				card_id = await this.model_extension_payment_sagepay_direct.addCard(card_data);
			} else if ((payment_data['Token'])) {
				card = await this.model_extension_payment_sagepay_direct.getCard(false, payment_data['Token']);
				card_id = card['card_id'];
			}

			await this.model_extension_payment_sagepay_direct.addOrder(this.session.data['order_id'], response_data, payment_data, card_id);
			await this.model_extension_payment_sagepay_direct.logger('Response data', response_data);
			await this.model_extension_payment_sagepay_direct.logger('payment_data', payment_data);
			await this.model_extension_payment_sagepay_direct.logger('order_id', this.session.data['order_id']);

			json['TermUrl'] = await this.url.link('extension/payment/sagepay_direct/callback', '', true);
			...
			sagepay_direct_order_id = await this.model_extension_payment_sagepay_direct.addOrder(order_info['order_id'], response_data, payment_data, card_id);
			await this.model_extension_payment_sagepay_direct.logger('Response data', response_data);
			await this.model_extension_payment_sagepay_direct.logger('payment_data', payment_data);
			await this.model_extension_payment_sagepay_direct.logger('order_id', this.session.data['order_id']);

			await this.model_extension_payment_sagepay_direct.addTransaction(sagepay_direct_order_id, this.config.get('payment_sagepay_direct_transaction'), order_info);

			await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_sagepay_direct_order_status_id'), message, false);
			...
			if (this.config.get('payment_sagepay_direct_transaction') == 'PAYMENT') {
				...
				for (recurring_products of item) {
					await this.model_extension_payment_sagepay_direct.recurringPayment(item, payment_data['VendorTxCode']);
				}
			}
			...
			await this.model_extension_payment_sagepay_direct.logger('Response data', json['error']);
		}
...
//				url = 'https://test+sagepay+com/gateway/service/direct3dcallback+vsp';
			...
			response_data = await this.model_extension_payment_sagepay_direct.sendCurl(url, this.request.post);
			await this.model_extension_payment_sagepay_direct.logger('response_data', response_data);
			...
			if (response_data['Status'] == 'OK' || response_data['Status'] == 'AUTHENTICATED' || response_data['Status'] == 'REGISTERED') {
				...
				order_info = await this.model_checkout_order.getOrder(this.session.data['order_id']);
				sagepay_order_info = await this.model_extension_payment_sagepay_direct.getOrder(this.session.data['order_id']);

				await this.model_extension_payment_sagepay_direct.logger('order_info', order_info);
				await this.model_extension_payment_sagepay_direct.logger('sagepay_order_info', sagepay_order_info);

				await this.model_extension_payment_sagepay_direct.updateOrder(order_info, response_data);
				await this.model_extension_payment_sagepay_direct.addTransaction(sagepay_order_info['sagepay_direct_order_id'], this.config.get('payment_sagepay_direct_transaction'), order_info);
				await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_sagepay_direct_order_status_id'), message, false);

				if (!empty(response_data['Token']) && await this.customer.isLogged()) {
					await this.model_extension_payment_sagepay_direct.updateCard(sagepay_order_info['card_id'], response_data['Token']);
				} else {
					await this.model_extension_payment_sagepay_direct.deleteCard(sagepay_order_info['card_id']);
				}
				...
				if (this.config.get('payment_sagepay_direct_transaction') == 'PAYMENT') {
					...
					//loop through any products that are recurring items
					for (recurring_products of item) {
						await this.model_extension_payment_sagepay_direct.recurringPayment(item, sagepay_order_info['VendorTxCode']);
					}
				}
	...
	async delete() {
		...
		card = await this.model_extension_payment_sagepay_direct.getCard(false, this.request.post['Token']);
...
//			payment_data['VPSProtocol'] = '3+00';
			...
			if (response_data['Status'] == 'OK') {
				await this.model_extension_payment_sagepay_direct.deleteCard(card['card_id']);
	...
	async cron() {
		if ((this.request.get['token']) && hash_equals(this.config.get('payment_sagepay_direct_cron_job_token'), this.request.get['token'])) {
			this.load.model('extension/payment/sagepay_direct');

			orders = await this.model_extension_payment_sagepay_direct.cronPayment();

			await this.model_extension_payment_sagepay_direct.updateCronJobRunTime();

			await this.model_extension_payment_sagepay_direct.logger('Repeat Orders', orders);
		}
	}
