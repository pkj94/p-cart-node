...
const data = {};
		...
		data['APPROVED_URL'] = await this.url.link('extension/payment/bluepay_hosted/callback', '', true);
		data['DECLINED_URL'] = await this.url.link('extension/payment/bluepay_hosted/callback', '', true);
		data['MISSING_URL'] = await this.url.link('extension/payment/bluepay_hosted/callback', '', true);
		data['REDIRECT_URL'] = await this.url.link('extension/payment/bluepay_hosted/callback', '', true);
		...
		return await this.load.view('extension/payment/bluepay_hosted', data);
	}
	...
	async callback() {
		...
		if ((this.session.data['order_id']) && this.config.get('payment_bluepay_hosted_status')) {
			order_info = await this.model_checkout_order.getOrder(this.session.data['order_id']);

			if (response_data['Result'] == 'APPROVED') {
				bluepay_hosted_order_id = await this.model_extension_payment_bluepay_hosted.addOrder(order_info, response_data);

				if (this.config.get('payment_bluepay_hosted_transaction') == 'SALE') {
					await this.model_extension_payment_bluepay_hosted.addTransaction(bluepay_hosted_order_id, 'payment', order_info);
				} else {
					await this.model_extension_payment_bluepay_hosted.addTransaction(bluepay_hosted_order_id, 'auth', order_info);
				}

				await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('payment_bluepay_hosted_order_status_id'));
