...
const data = {};
		...
		data['callback'] = await this.url.link('extension/payment/paypoint/callback', '', true);
		...
		return await this.load.view('extension/payment/paypoint', data);
	}

	async callback() {
		if ((this.request.get['trans_id'])) {
		...
		if (order_info) {
			...
			data['text_success_wait'] = sprintf(this.language.get('text_success_wait'), await this.url.link('checkout/success'));
			data['text_failure_wait'] = sprintf(this.language.get('text_failure_wait'), await this.url.link('checkout/cart'));

			if ((this.request.get['code']) && this.request.get['code'] == 'A' && status) {
				message = '';
				...
				if ((this.request.get['cv2avs'])) {
					message += 'cv2avs: ' + this.request.get['cv2avs'] + "\n";
				}

				if ((this.request.get['valid'])) {
					message += 'valid: ' + this.request.get['valid'] + "\n";
				}

				this.load.model('checkout/order',this);

				await this.model_checkout_order.addOrderHistory(order_id, this.config.get('payment_paypoint_order_status_id'), message, false);
				...
				this.response.setOutput(await this.load.view('extension/payment/paypoint_success', data));
			} else {
				...
				this.response.setOutput(await this.load.view('extension/payment/paypoint_failure', data));
			}
		}
	}
}