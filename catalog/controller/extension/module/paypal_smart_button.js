module.exports = class ControllerExtensionModulePayPalSmartButton extends Controller {
	...
	async index() {
const data = {};		
		if (this.config.get('payment_paypal_status') && (this.request.get['route'])) {						
			status = false;
			...
			setting = array_replace_recursive(config_setting, this.config.get('module_paypal_smart_button_setting'));
			...
			if (setting['page']['product']['status'] && (this.request.get['route'] == 'product/product') && (this.request.get['product_id'])) {
			...
			if (setting['page']['cart']['status'] && (this.request.get['route'] == 'checkout/cart') && this.cart.getTotal()) {
				...
				total = await this.cart.getTotal();
			...
			if (status) {				
				...
				return await this.load.view('extension/module/paypal_smart_button', data);
			}
		}
	}
	...
	async createOrder() {
		...
		if ((this.request.post['product_id'])) {
			...
			if (product_info.product_id) {
				...
				if (!errors) {					
					if (!await this.model_extension_module_paypal_smart_button.hasProductInCart(this.request.post['product_id'], option, recurring_id)) {
		...
		if (!errors) {					
			...
			if (paypal.hasErrors()) {
				...
				for (errors of error) {
					...
					await this.model_extension_module_paypal_smart_button.log(error, error['message']);
				}
	...
	async approveOrder() {
		...
		if (order_info && !this.error) {
			...
			if (await this.customer.isLogged()) {
				...
				this.session.data['guest']['firstname'] = ((order_info['payer']['name']['given_name']) ? order_info['payer']['name']['given_name'] : '');
				this.session.data['guest']['lastname'] = ((order_info['payer']['name']['surname']) ? order_info['payer']['name']['surname'] : '');
				this.session.data['guest']['email'] = ((order_info['payer']['email_address']) ? order_info['payer']['email_address'] : '');
			...
			if (await this.customer.isLogged() && await this.customer.getAddressId()) {
				...
				this.session.data['payment_address']['firstname'] = ((order_info['payer']['name']['given_name']) ? order_info['payer']['name']['given_name'] : '');
				this.session.data['payment_address']['lastname'] = ((order_info['payer']['name']['surname']) ? order_info['payer']['name']['surname'] : '');
				...
				if ((order_info['payer']['address']['country_code'])) {
					country_info = await this.model_extension_module_paypal_smart_button.getCountryByCode(order_info['payer']['address']['country_code']);
			...
			if (await this.cart.hasShipping()) {
				if (await this.customer.isLogged() && await this.customer.getAddressId()) {
					...
					if ((order_info['purchase_units'][0]['shipping']['name']['full_name'])) {
					...
					this.session.data['shipping_address']['address_1'] = ((order_info['purchase_units'][0]['shipping']['address']['address_line_1']) ? order_info['purchase_units'][0]['shipping']['address']['address_line_1'] : '');
					this.session.data['shipping_address']['address_2'] = ((order_info['purchase_units'][0]['shipping']['address']['address_line_2']) ? order_info['purchase_units'][0]['shipping']['address']['address_line_2'] : '');
					this.session.data['shipping_address']['city'] = ((order_info['purchase_units'][0]['shipping']['address']['admin_area_2']) ? order_info['purchase_units'][0]['shipping']['address']['admin_area_2'] : '');
					this.session.data['shipping_address']['postcode'] = ((order_info['purchase_units'][0]['shipping']['address']['postal_code']) ? order_info['purchase_units'][0]['shipping']['address']['postal_code'] : '');
					...
					if ((order_info['purchase_units'][0]['shipping']['address']['country_code'])) {
						country_info = await this.model_extension_module_paypal_smart_button.getCountryByCode(order_info['purchase_units'][0]['shipping']['address']['country_code']);
						...
						if (country_info) {
							...
							if ((order_info['purchase_units'][0]['shipping']['address']['admin_area_1'])) {
								zone_info = await this.model_extension_module_paypal_smart_button.getZoneByCode(country_info['country_id'], order_info['purchase_units'][0]['shipping']['address']['admin_area_1']);
			...
			data['url'] = await this.url.link('extension/module/paypal_smart_button/confirmOrder', '', true);			
		}
	...
	async confirmOrder() {
		...
		if ((this.request.post['coupon']) && this.validateCoupon()) {
			...
			this.response.setRedirect(await this.url.link('extension/module/paypal_smart_button/confirmOrder', '', true));
		}
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/module/paypal_smart_button/confirmOrder', '', true)
		});
			...
			if (!empty(data['shipping_address'])) {
				...
				if (!empty(results)) {
					...
					if (!empty(quote_data)) {
						...
						data['action_shipping'] = await this.url.link('extension/module/paypal_smart_button/confirmShipping', '', true);
					} else {
						delete this.session.data['shipping_methods']);
						delete this.session.data['shipping_method']);
		...
		data['action_confirm'] = await this.url.link('extension/module/paypal_smart_button/completeOrder', '', true);
		...
		data['coupon'] = await this.load.controller('extension/total/coupon');
		data['voucher'] = await this.load.controller('extension/total/voucher');
		data['reward'] = await this.load.controller('extension/total/reward');
		...
		this.response.setOutput(await this.load.view('extension/module/paypal_smart_button/confirm', data));
	}
	...
	async completeOrder() {		
		...
		if ((this.session.data['paypal_order_id'])) {			
			...
			if (transaction_method == 'authorize') {
				...
				if ((result['purchase_units'][0]['payments']['authorizations'][0]['seller_protection'])) {
				...
				if ((result['purchase_units'][0]['payments']['captures'][0]['seller_protection'])) {
			...
			if (!Object.keys(this.error).length) {								
				...
				await this.model_checkout_order.addOrderHistory(this.session.data['order_id'], this.config.get('config_order_status_id'), message);
	...
	async paymentAddress() {
		...
		this.response.setOutput(await this.load.view('extension/module/paypal_smart_button/payment_address', data));
	}
	...
	async shippingAddress() {
		...
		this.response.setOutput(await this.load.view('extension/module/paypal_smart_button/shipping_address', data));
	}
