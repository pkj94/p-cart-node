...
const data = {};
        ...
        if (this.config.get('payment_amazon_login_pay_status') && this.config.get('module_amazon_pay_status') && !empty(this.request.server['HTTPS']) && !(this.config.get('payment_amazon_login_pay_minimum_total') > 0 && this.config.get('payment_amazon_login_pay_minimum_total') > this.cart.getSubTotal()) && await this.model_extension_payment_amazon_login_pay.isTotalPositive()) {
            ...
            data['return_url'] = html_entity_decode(await this.url.link('extension/module/amazon_login/login', 'from_amazon_pay=1', true), ENT_COMPAT, "UTF-8");
