module.exports = class OrderController extends Controller {
	...
	async info() {
		...
		order_info = await this.model_account_order.getOrder(order_id);
		...
		if (order_info) {
			...
			for (let product of products) {
				...
				options = await this.model_account_order.getOptions(order_id, product['order_product_id']);
				...
				subscription_info = await this.model_account_order.getSubscription(order_id, product['order_product_id']);
			...
			vouchers = await this.model_account_order.getVouchers(order_id);
			...
			totals = await this.model_account_order.getTotals(order_id);
	...
	async reorder() {
		...
		if (order_info) {
			...
			order_product_info = await this.model_account_order.getProduct(order_id, order_product_id);
			...
			if (order_product_info) {
				...
				if (product_info) {
					...
					subscription_info = await this.model_account_order.getSubscription(order_product_info['order_id'], order_product_id);
