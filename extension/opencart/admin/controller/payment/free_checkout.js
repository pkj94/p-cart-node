global['\Opencart\Admin\Controller\Extension\Opencart\Payment\FreeCheckout'] = class FreeCheckout extends global['\Opencart\System\Engine\Controller'] {
	...
	async index() {
		...
		data.breadcrumbs.push({
			...
			href: await this.url.link('extension/opencart/payment/free_checkout', 'user_token=' + this.session.data.user_token)
		});

		data.save = await this.url.link('extension/opencart/payment/free_checkout.save', 'user_token=' + this.session.data.user_token);
