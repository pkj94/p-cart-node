...
global['\Opencart\Admin\Controller\Extension\Opencart\Report\SaleTax'] = class SaleTax extends global['\Opencart\System\Engine\Controller'] {
	...
	async index() {
		const data = {};
		await this.load.language('extension/opencart/report/sale_tax');
		...
		data['breadcrumbs'].push({
			...
			'href': await this.url.link('extension/opencart/report/sale_tax', 'user_token=' + this.session.data['user_token'])
		});

		data['save'] = await this.url.link('extension/opencart/report/sale_tax.save', 'user_token=' + this.session.data['user_token']);
		...
		this.response.setOutput(await this.load.view('extension/opencart/report/sale_tax_form', data));
	}
	...
	async save() {
		...
		if (!json.error) {
			...
			await this.model_setting_setting.editSetting('report_sale_tax', this.request.post);
	...
	async report() {
		...
		this.response.setOutput(await this.load.view('extension/opencart/report/sale_tax', data));
	}
	...
	async getReport() {
		...
		const order_total = await this.model_extension_opencart_report_sale.getTotalTaxes(filter_data);

		data['orders'] = [];

		const results = await this.model_extension_opencart_report_sale.getTaxes(filter_data);
		...
		data['pagination'] = await this.load.controller('common/pagination', {
			...
			'page': page,
			'limit': this.config.get('config_pagination'),
			'url': await this.url.link('extension/opencart/report/sale_tax.report', 'user_token=' + this.session.data['user_token'] + '&code=sale_tax' + url + '&page={page}')
		});
		...
		return await this.load.view('extension/opencart/report/sale_tax_list', data);
	}
}
