...
global['OpencartAdminControllerExtensionOpencartReportSaleOrder'] = class SaleOrder extends global['OpencartSystemEngineController'] {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href': await this.url.link('extension/opencart/report/sale_order', 'user_token=' + this.session.data['user_token'])
		});

		data['save'] = await this.url.link('extension/opencart/report/sale_order.save', 'user_token=' + this.session.data['user_token']);
		...
		this.response.setOutput(await this.load.view('extension/opencart/report/sale_order_form', data));
	}
	...
	async report() {
		...
		this.response.setOutput(await this.load.view('extension/opencart/report/sale_order', data));
	}
	...
	async getReport() {
		...
		data['pagination'] = await this.load.controller('common/pagination', {
			...
			'page': page,
			'limit': this.config.get('config_pagination'),
			'url': await this.url.link('extension/opencart/report/sale_order', 'user_token=' + this.session.data['user_token'] + '&code=sale_order' + url + '&page={page}')
		});
		...
		return await this.load.view('extension/opencart/report/sale_order_list', data);
	}
}
