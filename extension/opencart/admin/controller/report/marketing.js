...
global['\Opencart\Admin\Controller\Extension\Opencart\Report\Marketing'] = class Marketing extends global['\Opencart\System\Engine\Controller'] {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href': await this.url.link('extension/opencart/report/marketing', 'user_token=' + this.session.data['user_token'])
		});

		data['save'] = await this.url.link('extension/opencart/report/marketing.save', 'user_token=' + this.session.data['user_token']);
	...
	async getReport() {
		...
		for (let result of results) {
			data['marketings'].push({
				...
				'save': await this.url.link('marketing/marketing/edit', 'user_token=' + this.session.data['user_token'] + '&marketing_id=' + result['marketing_id'])
			});
		}
		...
		data['pagination'] = await this.load.controller('common/pagination', {
			...
			'page': page,
			'limit': this.config.get('config_pagination'),
			'url': await this.url.link('extension/opencart/report/marketing.report', 'user_token=' + this.session.data['user_token'] + '&code=marketing' + url + '&page={page}')
		});
