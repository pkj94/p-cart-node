...
global['\Opencart\Admin\Controller\Extension\Opencart\Report\SaleCoupon'] = class SaleCoupon extends global['\Opencart\System\Engine\Controller'] {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href': await this.url.link('extension/opencart/report/sale_coupon', 'user_token=' + this.session.data['user_token'])
		});

		data['save'] = await this.url.link('extension/opencart/report/sale_coupon.save', 'user_token=' + this.session.data['user_token']);
	...
	async getReport() {
		...
		for (let result of results) {
			data['coupons'].push({
				...
				'edit': await this.url.link('marketing/coupon.edit', 'user_token=' + this.session.data['user_token'] + '&coupon_id=' + result['coupon_id'])
			});
		}
		...
		data['pagination'] = await this.load.controller('common/pagination', {
			...
			'page': page,
			'limit': this.config.get('config_pagination'),
			'url': await this.url.link('extension/opencart/report/sale_coupon.report', 'user_token=' + this.session.data['user_token'] + '&code=sale_coupon' + url + '&page={page}')
		});
