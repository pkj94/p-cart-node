global['\Opencart\Admin\Controller\Extension\Opencart\Total\Reward'] = class Reward extends global['\Opencart\System\Engine\Controller'] {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/opencart/total/reward', 'user_token=' + this.session.data['user_token'])
		});

		data['save'] = await this.url.link('extension/opencart/total/reward.save', 'user_token=' + this.session.data['user_token']);
