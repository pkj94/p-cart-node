global['\Opencart\Admin\Controller\Extension\Opencart\Total\Total'] = class Total extends Controller {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/opencart/total/total', 'user_token=' + this.session.data['user_token'])
		});

		data['save'] = await this.url.link('extension/opencart/total/total.save', 'user_token=' + this.session.data['user_token']);
