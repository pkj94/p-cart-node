...
class Cheque extends \Opencart\System\Engine\Model {
	...
	async getMethods(array $address = []): array {
		...
		if (this.cart.hasSubscription()) {
			...
			$query = this.db.query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . this.config.get('payment_cheque_geo_zone_id') . "' AND `country_id` = '" . $address['country_id'] . "' AND (`zone_id` = '" . $address['zone_id'] . "' OR `zone_id` = '0')");
