...
class Coupon extends \Opencart\System\Engine\Model {
	...
	async confirm(array $order_info, array $order_total): int {
		...
		if ($code) {
			...
			if ($status) {
				this.db.query("INSERT INTO `" . DB_PREFIX . "coupon_history` SET `coupon_id` = '" . $coupon_query.row['coupon_id'] . "', `order_id` = '" . $order_info['order_id'] . "', `customer_id` = '" . $order_info['customer_id'] . "', `amount` = '" . $order_total['value'] . "', `date_added` = NOW()");
			} else {
