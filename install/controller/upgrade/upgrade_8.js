global['\Opencart\Install\Controller\Upgrade\Upgrade8'] = class Upgrade8 extends global['\Opencart\System\Engine\Controller'] {
	/**
	 * @return void
	 */
	async index() {
		...
		try {
			...
			if (query.num_rows) {
				...
				for (customer_group_query.rows of customer_group) {
			...
			if (query.num_rows) {
				...
				config = new global['\Opencart\System\Engine\Config']();

				setting_query = this.db.query("SELECT * FROM `" + DB_PREFIX + "setting` WHERE `store_id` = '0'");

				for (setting_query.rows of setting) {
				...
				for (affiliate_query.rows of affiliate) {
					...
					this.db.query("UPDATE `" + DB_PREFIX + "order` SET `affiliate_id` = '" + customer_id + "' WHERE `affiliate_id` = '" + affiliate['affiliate_id'] + "'");
				}
			}
