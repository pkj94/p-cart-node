global['\Opencart\Install\Controller\Upgrade\Upgrade9'] = class Upgrade9 extends Controller {
	/**
	 * @return void
	 */
	async index() {
			...
			if (query.num_rows) {
				...
				for (query.rows of result) {
					...
					if (isset(result['shipping_code'])) {
						order_total_query = this.db.query("SELECT * FROM `" + DB_PREFIX + "order_total` WHERE `order_id` = '" + result['order_id'] + "' AND `code` = 'shipping'");
