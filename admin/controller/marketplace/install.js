module.exports = class ControllerMarketplaceInstall extends Controller {
	async install() {
		await this.load.language('marketplace/install');
		...
		if (!await this.user.hasPermission('modify', 'marketplace/install')) {
			json['error'] = this.language.get('error_permission');
		}

		// Make sure the file name is stored in the session.
		if (!(this.session.data['install'])) {
			json['error'] = this.language.get('error_file');
		} else if (!is_file(DIR_UPLOAD + this.session.data['install'] + '.tmp')) {
			json['error'] = this.language.get('error_file');
		}
		...
		if (!json) {
			...
			json['next'] = str_replace('&amp;', '&', await this.url.link('marketplace/install/unzip', 'user_token=' + this.session.data['user_token'] + '&extension_install_id=' + extension_install_id, true));
		}
	...
	async unzip() {
		...
		if (!json) {
			...
			json['next'] = str_replace('&amp;', '&', await this.url.link('marketplace/install/move', 'user_token=' + this.session.data['user_token'] + '&extension_install_id=' + extension_install_id, true));
		}
	...
	async move() {
		...
		} else if (!is_dir(DIR_UPLOAD + 'tmp-' + this.session.data['install'] + '/')) {
			json['error'] = this.language.get('error_directory');
		}
				...
				if (!json) {
					...
					for (let file of files) {
						...
						if (is_dir(file) && !is_dir(path)) {
							if (mkdir(path, 0777)) {
								await this.model_setting_extension.addExtensionPath(extension_install_id, destination);
							}
						}
		...
		if (!json) {
			...
			json['next'] = str_replace('&amp;', '&', await this.url.link('marketplace/install/xml', 'user_token=' + this.session.data['user_token'] + '&extension_install_id=' + extension_install_id, true));
		}
	...
	async xml() {
				...
				if (xml) {
					try {
						...
						if (code) {
							...
							modification_info = await this.model_setting_modification.getModificationByCode(code);
	
							if (modification_info) {
								await this.model_setting_modification.deleteModification(modification_info['modification_id']);
							}
						} else {
		...
		if (!json) {
			...
			json['next'] = str_replace('&amp;', '&', await this.url.link('marketplace/install/remove', 'user_token=' + this.session.data['user_token'], true));
		}
	...
	async uninstall() {
		...
		if (!json) {
			...
			results = await this.model_setting_extension.getExtensionPathsByExtensionInstallId(extension_install_id);
			...
			for (let result of results) {
				...
				await this.model_setting_extension.deleteExtensionPath(result['extension_path_id']);
			}
	...
	private function isDirEmpty (dir_name) {
		...
		for (scandir(dir_name) of dir_file)
		{
