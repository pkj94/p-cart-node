module.exports = class ControllerSaleRecurring extends Controller {
	...
	async getList() {
		...
		const recurrings_total = await this.model_sale_recurring.getTotalRecurrings(filter_data);

		const results = await this.model_sale_recurring.getRecurrings(filter_data);
	...
	async info() {
		...
		const order_recurring_info = await this.model_sale_recurring.getRecurring(order_recurring_id);
		...
		if (order_recurring_info.order_recurring_id) {
			...
			const order_info = await this.model_sale_order.getOrder(order_recurring_info['order_id']);
			...
			const transactions = await this.model_sale_recurring.getRecurringTransactions(order_recurring_info['order_recurring_id']);
