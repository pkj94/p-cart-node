module.exports = class ControllerExtensionShippingParcelforce48 extends Controller {
	error = {};

	async index() {
const data = {};
		await this.load.language('extension/shipping/parcelforce_48');
		...
		if ((this.request.server['method'] == 'POST') && await this.validate()) {
			await this.model_setting_setting.editSetting('shipping_parcelforce_48', this.request.post);
...
await this.session.save(this.session.data);
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/shipping/parcelforce_48', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/shipping/parcelforce_48', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=shipping', true);

		if ((this.request.post['shipping_parcelforce_48_rate'])) {
			data['shipping_parcelforce_48_rate'] = this.request.post['shipping_parcelforce_48_rate'];
		} else if (this.config.get('shipping_parcelforce_48_rate')) {
		...
		if ((this.request.post['shipping_parcelforce_48_insurance'])) {
			data['shipping_parcelforce_48_insurance'] = this.request.post['shipping_parcelforce_48_insurance'];
		} else if (this.config.get('shipping_parcelforce_48_insurance')) {
		...
		if ((this.request.post['shipping_parcelforce_48_display_weight'])) {
		...
		if ((this.request.post['shipping_parcelforce_48_display_insurance'])) {
		...
		if ((this.request.post['shipping_parcelforce_48_display_time'])) {
		...
		if ((this.request.post['shipping_parcelforce_48_tax_class_id'])) {
		...
		if ((this.request.post['shipping_parcelforce_48_geo_zone_id'])) {
		...
		if ((this.request.post['shipping_parcelforce_48_status'])) {
		...
		if ((this.request.post['shipping_parcelforce_48_sort_order'])) {
		...
		this.response.setOutput(await this.load.view('extension/shipping/parcelforce_48', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/shipping/parcelforce_48')) {
