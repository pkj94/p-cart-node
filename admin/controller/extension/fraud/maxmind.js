module.exports = class ControllerExtensionFraudMaxMind extends Controller {
	error = {};
	...
	async index() {
		...
		if ((this.request.post['fraud_maxmind_score'])) {
		...
		if ((this.request.post['fraud_maxmind_order_status_id'])) {
		...
		if ((this.request.post['fraud_maxmind_status'])) {
		...
		this.response.setOutput(await this.load.view('extension/fraud/maxmind', data));
	}
	...
	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/fraud/maxmind')) {
	...
	async order() {
		...
		if (fraud_info) {
			...
			return await this.load.view('extension/fraud/maxmind_info', data);
		}
	}
}