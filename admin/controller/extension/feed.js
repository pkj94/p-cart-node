...
const fs= require('fs');
module.exports = class FeedController extends global['\Opencart\System\Engine\Controller'] {
	/**
	 * @return void
	 */
	async index() {
	...
	async getList() {
		...
		const results = await this.model_setting_extension.getPaths('%/admin/controller/feed/%.js');
		...
		const extensions = await this.model_setting_extension.getExtensionsByType('feed');
		...
		if (results) {
			for (let result of results) {
				...
				data['extensions'].push({
					...
					'install': await this.url.link('extension/feed.install', 'user_token=' + this.session.data['user_token'] + '&extension=' + extension + '&code=' + code),
					'uninstall': await this.url.link('extension/feed.uninstall', 'user_token=' + this.session.data['user_token'] + '&extension=' + extension + '&code=' + code),
					'installed': installed.includes(code),
					'edit': await this.url.link('extension/' + extension + '/feed/' + code, 'user_token=' + this.session.data['user_token'])
				});
			}
		}
	...
	async install() {
		...
		if (!fs.existsSync(DIR_EXTENSION + extension + '/admin/controller/feed/' + code + '.js')) {
...
this.autoloader.register('Opencart\Admin\Controller\Extension\\' +namespace, DIR_EXTENSION + extension + '/admin/controller/');
			...
			await this.load.controller('extension/' + extension + '/feed/' + code + '.install');
	...
	async uninstall() {
		...
		if (!Object.keys(json).length) {
			...
			await this.load.controller('extension/' + this.request.get['extension'] + '/feed/' + this.request.get['code'] + '.uninstall');
