module.exports = class ControllerExtensionReportSaleTax extends Controller {
	error = {};

	async index() {
		const data = {};
		await this.load.language('extension/report/sale_tax');
		...
		data['breadcrumbs'].push({
			...
			'href': await this.url.link('extension/report/sale_tax', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/report/sale_tax', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=report', true);

		if ((this.request.post['report_sale_tax_status'])) {
		...
		if ((this.request.post['report_sale_tax_sort_order'])) {
		...
		this.response.setOutput(await this.load.view('extension/report/sale_tax_form', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/report/sale_tax')) {
	...
	async report() {
		...
		this.load.model('extension/report/sale', this);
		...
		const order_total = await this.model_extension_report_sale.getTotalTaxes(filter_data);

		data['orders'] = [];

		const results = await this.model_extension_report_sale.getTaxes(filter_data);
		...
		pagination.url = await this.url.link('report/report', 'user_token=' + this.session.data['user_token'] + '&code=sale_tax' + url + '&page={page}', true);
		...
		return await this.load.view('extension/report/sale_tax_info', data);
	}
}