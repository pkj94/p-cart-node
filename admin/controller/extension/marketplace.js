...
const fs= require('fs');
module.exports = class MarketplaceController extends global['\Opencart\System\Engine\Controller'] {
	/**
	 * @return void
	 */
	async index() {
		...
		if (results) {
			for (let result of results) {
				...
				data['extensions'].push({
					...
					'install': await this.url.link('extension/marketplace.install', 'user_token=' + this.session.data['user_token'] + '&extension=' + extension + '&code=' + code),
					'uninstall': await this.url.link('extension/marketplace.uninstall', 'user_token=' + this.session.data['user_token'] + '&extension=' + extension + '&code=' + code),
					'installed': installed.includes(code),
					'edit': await this.url.link('extension/' + extension + '/marketplace/' + code, 'user_token=' + this.session.data['user_token'])
				});
			}
		}
	...
	async install() {
		...
		if (!Object.keys(json).length) {
			...
			await this.model_user_user_group.addPermission(await this.user.getGroupId(), 'access', 'extension/' + extension + '/marketplace/' + code);
			await this.model_user_user_group.addPermission(await this.user.getGroupId(), 'modify', 'extension/' + extension + '/marketplace/' + code);
