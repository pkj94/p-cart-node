module.exports = class ControllerExtensionTotalReward extends Controller {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/total/reward', 'user_token=' + this.session.data['user_token'], true)
		);

		data['action'] = await this.url.link('extension/total/reward', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=total', true);

		if ((this.request.post['total_reward_status'])) {
		...
		if ((this.request.post['total_reward_sort_order'])) {
		...
		this.response.setOutput(await this.load.view('extension/total/reward', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/total/reward')) {
