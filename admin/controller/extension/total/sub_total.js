module.exports = class ControllerExtensionTotalSubTotal extends Controller {
	error = {};
...
await this.session.save(this.session.data);
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/total/sub_total', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/total/sub_total', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=total', true);

		if ((this.request.post['total_sub_total_status'])) {
		...
		if ((this.request.post['total_sub_total_sort_order'])) {
		...
		this.response.setOutput(await this.load.view('extension/total/sub_total', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/total/sub_total')) {
