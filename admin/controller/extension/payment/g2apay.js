...
module.exports =
	class ControllerExtensionPaymentG2APay extends Controller {
		...
		async index() {
			...
			data['g2apay_ipn_url'] = HTTPS_CATALOG + '?route=extension/payment/g2apay/ipn&token=' + data['payment_g2apay_secret_token'];
		...
		async order() {
			...
			if (this.config.get('payment_g2apay_status')) {
				...
				this.load.model('extension/payment/g2apay', this);

				const g2apay_order = await this.model_extension_payment_g2apay.getOrder(this.request.get['order_id']);

				if ((g2apay_order.g2apay_order_id)) {
					await this.load.language('extension/payment/g2apay');
		...
		async refund() {
			...
			if ((this.request.post['order_id']) && (this.request.post['order_id'])) {
				...
				const g2apay_order = await this.model_extension_payment_g2apay.getOrder(this.request.post['order_id']);

				const refund_response = await this.model_extension_payment_g2apay.refund(g2apay_order, this.request.post['amount']);
				...
				if (refund_response == 'ok') {
					...
					const total_refunded = await this.model_extension_payment_g2apay.getTotalRefunded(g2apay_order['g2apay_order_id']);
					const total_released = await this.model_extension_payment_g2apay.getTotalReleased(g2apay_order['g2apay_order_id']);
					let refund_status = 0;
