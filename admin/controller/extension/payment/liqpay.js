module.exports = class ControllerExtensionPaymentLiqPay extends Controller {
	error = {};
	...
	async index() {
		...
		if ((this.request.server['method'] == 'POST') && await this.validate()) {
			await this.model_setting_setting.editSetting('payment_liqpay', this.request.post);
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/payment/liqpay', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/payment/liqpay', 'user_token=' + this.session.data['user_token'], true);
		...
		if ((this.request.post['payment_liqpay_type'])) {
		...
		if ((this.request.post['payment_liqpay_total'])) {
		...
		if ((this.request.post['payment_liqpay_order_status_id'])) {
		...
		if ((this.request.post['payment_liqpay_geo_zone_id'])) {
		...
		if ((this.request.post['payment_liqpay_status'])) {
		...
		if ((this.request.post['payment_liqpay_sort_order'])) {
		...
		this.response.setOutput(await this.load.view('extension/payment/liqpay', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/payment/liqpay')) {
