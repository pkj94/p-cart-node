module.exports = class ControllerExtensionPaymentSagepayServer extends Controller {
	...
	async index() {
		...
		if ((this.request.post['payment_sagepay_server_password'])) {
		...
		if ((this.request.post['payment_sagepay_server_test'])) {
		...
		if ((this.request.post['payment_sagepay_server_transaction'])) {
		...
		if ((this.request.post['payment_sagepay_server_total'])) {
		...
		if ((this.request.post['payment_sagepay_server_card'])) {
		...
		if ((this.request.post['payment_sagepay_server_order_status_id'])) {
		...
		if ((this.request.post['payment_sagepay_server_cron_job_token'])) {
			data['payment_sagepay_server_cron_job_token'] = this.request.post['payment_sagepay_server_cron_job_token'];
		} else if (this.config.get('payment_sagepay_server_cron_job_token')) {
		...
		if ((this.request.post['payment_sagepay_server_geo_zone_id'])) {
		...
		if ((this.request.post['payment_sagepay_server_status'])) {
		...
		if ((this.request.post['payment_sagepay_server_debug'])) {
		...
		if ((this.request.post['payment_sagepay_server_sort_order'])) {
		...
		this.response.setOutput(await this.load.view('extension/payment/sagepay_server', data));
	}
	...
	async order() {
		if (this.config.get('payment_sagepay_server_status')) {
			...
			if ((sagepay_server_order)) {
				...
				sagepay_server_order['total_released'] = await this.model_extension_payment_sagepay_server.getTotalReleased(sagepay_server_order['sagepay_server_order_id']);
	...
	async void() {
		...
		if ((this.request.post['order_id']) && this.request.post['order_id'] != '') {
			...
			if (void_response['Status'] == 'OK') {
				await this.model_extension_payment_sagepay_server.addTransaction(sagepay_server_order['sagepay_server_order_id'], 'void', 0.00);
				await this.model_extension_payment_sagepay_server.updateVoidStatus(sagepay_server_order['sagepay_server_order_id'], 1);
	...
	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/payment/sagepay_server')) {
