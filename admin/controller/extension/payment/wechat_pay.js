...
class ControllerExtensionPaymentWechatPay extends Controller {
	...
	async index() {
		...
		if ((this.error['mch_id'])) {
			data['error_mch_id'] = this.error['mch_id'];
		} else {
			data['error_mch_id'] = '';
		}

		if ((this.error['api_secret'])) {
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/payment/wechat_pay', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/payment/wechat_pay', 'user_token=' + this.session.data['user_token'], true);
		...
		if ((this.request.post['payment_wechat_pay_total'])) {
		...
		if ((this.request.post['payment_wechat_pay_currency'])) {
		...
		if ((this.request.post['payment_wechat_pay_completed_status_id'])) {
		...
		if ((this.request.post['payment_wechat_pay_geo_zone_id'])) {
		...
		if ((this.request.post['payment_wechat_pay_status'])) {
		...
		if ((this.request.post['payment_wechat_pay_sort_order'])) {
