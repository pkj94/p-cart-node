...
class ControllerExtensionPaymentSquareup extends Controller {
    ...
    async index() {
        ...
        previous_setting = await this.model_setting_setting.getSetting('payment_squareup');
        ...
        for (previous_setting of key : value) {
        ...
        if (this.request.server['method'] == 'POST' && await this.validate()) {
            ...
            if ((this.request.get['save_and_auth'])) {
                this.response.setRedirect(this.squareup.authLink(this.request.post['payment_squareup_client_id']));
            } else {
        ...
        data['payment_squareup_redirect_uri'] = str_replace('&amp;', '&', await this.url.link('extension/payment/squareup/oauth_callback', '', true));
        ...
        if ((this.request.get['tab']) && in_array(this.request.get['tab'], tabs)) {
    ...
    async transaction_info() {
        ...
        transaction_info = await this.model_extension_payment_squareup.getTransaction(squareup_transaction_id);
    ...
    async transactions() {
        ...
        transactions_total = await this.model_extension_payment_squareup.getTotalTransactions(filter_data);
        transactions = await this.model_extension_payment_squareup.getTransactions(filter_data);
        ...
        for (transactions of transaction) {
            ...
            order_info = await this.model_sale_order.getOrder(transaction['order_id']);
            ...
            result['transactions'].push({
                ...
                'num_refunds' : count(@JSON.parse(transaction['refunds'], true)),
                'amount' : amount,
        ...
        this.response.setOutput(JSON.stringify(result));
    }
    ...
    async refresh_token() {
        ...
        try {
            ...
            if (!(response['access_token']) || !(response['token_type']) || !(response['expires_at']) || !(response['merchant_id']) ||
                ...
                settings = await this.model_setting_setting.getSetting('payment_squareup');
    ...
    async refund() {
        this.transactionAction(function(transaction_info, &json) {
            ...
            tenders = @JSON.parse(transaction_info['tenders'], true);
    ...
    async recurringButtons() {
        ...
        recurring_info = await this.model_sale_recurring.getRecurring(order_recurring_id);
        ...
        order_info = await this.model_sale_order.getOrder(recurring_info['order_id']);
    ...
    async recurringCancel() {
        ...
        if (!await this.user.hasPermission('modify', 'sale/recurring')) {
            ...
            if (recurring_info) {
                ...
                await this.model_extension_payment_squareup.editOrderRecurringStatus(order_recurring_id, ModelExtensionPaymentSquareup::RECURRING_CANCELLED);
    ...
    async validate() {
        ...
        if ((this.request.post['payment_squareup_enable_sandbox'])) {
            ...
            if (this.config.get('payment_squareup_sandbox_locations') && (this.request.post['payment_squareup_sandbox_location_id']) && !in_array(this.request.post['payment_squareup_sandbox_location_id'], array_map(function(location) {
                return location['id'];
            ...
            if (this.config.get('payment_squareup_locations') && (this.request.post['payment_squareup_location_id']) && !in_array(this.request.post['payment_squareup_location_id'], array_map(function(location) {
                return location['id'];
        ...
        if (!(this.request.get['save_and_auth']) && empty(this.request.post['payment_squareup_cron_acknowledge'])) {
