...
module.exports = class ControllerExtensionPaymentWorldpay extends Controller {
	...
	async index() {
		...
		data['payment_worldpay_webhook_url'] = HTTPS_CATALOG + '?route=extension/payment/worldpay/webhook&token=' + data['payment_worldpay_secret_token'];

		data['payment_worldpay_cron_job_url'] = HTTPS_CATALOG + '?route=extension/payment/worldpay/cron&token=' + data['payment_worldpay_secret_token'];
	...
	async order() {
		const data = {};
		if (Number(this.config.get('payment_worldpay_status'))) {

			this.load.model('extension/payment/worldpay', this);

			const worldpay_order = await this.model_extension_payment_worldpay.getOrder(this.request.get['order_id']);

			if ((worldpay_order.payment_worldpay_order_id)) {
				await this.load.language('extension/payment/worldpay');
	...
	async refund() {
		...
		if ((this.request.post['order_id']) && (this.request.post['order_id'])) {
			...
			const worldpay_order = await this.model_extension_payment_worldpay.getOrder(this.request.post['order_id']);

			const refund_response = await this.model_extension_payment_worldpay.refund(this.request.post['order_id'], this.request.post['amount']);

			await this.model_extension_payment_worldpay.logger('Refund result: ' + JSON.stringify(refund_response, 1));
			...
			if (refund_response['status'] == 'success') {
				...
				const total_refunded = await this.model_extension_payment_worldpay.getTotalRefunded(worldpay_order['payment_worldpay_order_id']);
				const total_released = await this.model_extension_payment_worldpay.getTotalReleased(worldpay_order['payment_worldpay_order_id']);
