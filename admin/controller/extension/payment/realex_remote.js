module.exports = class ControllerExtensionPaymentRealexRemote extends Controller {
	error = {};

	async index() {
		const data = {};
		await this.load.language('extension/payment/realex_remote');
		...
		if ((this.request.server['method'] == 'POST') && await this.validate()) {
			await this.model_setting_setting.editSetting('payment_realex_remote', this.request.post);
		...
		if ((this.request.post['payment_realex_remote_rebate_password'])) {
		...
		if ((this.request.post['payment_realex_remote_geo_zone_id'])) {
		...
		if ((this.request.post['payment_realex_remote_total'])) {
		...
		if ((this.request.post['payment_realex_remote_sort_order'])) {
		...
		if ((this.request.post['payment_realex_remote_status'])) {
		...
		if ((this.request.post['payment_realex_remote_card_data_status'])) {
		...
		if ((this.request.post['payment_realex_remote_debug'])) {
		...
		if ((this.request.post['payment_realex_remote_account'])) {
		...
		if ((this.request.post['payment_realex_remote_auto_settle'])) {
		...
		if ((this.request.post['payment_realex_remote_tss_check'])) {
		...
		if ((this.request.post['payment_realex_remote_3d'])) {
		...
		if ((this.request.post['payment_realex_remote_liability'])) {
		...
		if ((this.request.post['payment_realex_remote_order_status_success_settled_id'])) {
		...
		if ((this.request.post['payment_realex_remote_order_status_success_unsettled_id'])) {
		...
		if ((this.request.post['payment_realex_remote_order_status_decline_id'])) {
		...
		if ((this.request.post['payment_realex_remote_order_status_decline_pending_id'])) {
		...
		if ((this.request.post['payment_realex_remote_order_status_decline_stolen_id'])) {
		...
		if ((this.request.post['payment_realex_remote_order_status_decline_bank_id'])) {
		...
		this.response.setOutput(await this.load.view('extension/payment/realex_remote', data));
	}

	async install() {
		this.load.model('extension/payment/realex_remote', this);
		await this.model_extension_payment_realex_remote.install();
	}
	...
	async order() {
		if (this.config.get('payment_realex_remote_status')) {
			...
			const realex_order = await this.model_extension_payment_realex_remote.getOrder(this.request.get['order_id']);

			if ((realex_order.realex_remote_order_id)) {
				await this.load.language('extension/payment/realex_remote');

				realex_order['total_captured'] = await this.model_extension_payment_realex_remote.getTotalCaptured(realex_order['realex_remote_order_id']);
				...
				return await this.load.view('extension/payment/realex_remote_order', data);
			}
		}
	}
	...
	async void() {
		...
		if ((this.request.post['order_id']) && this.request.post['order_id'] != '') {
			...
			const realex_order = await this.model_extension_payment_realex_remote.getOrder(this.request.post['order_id']);

			const void_response = await this.model_extension_payment_realex_remote.void(this.request.post['order_id']);

			await this.model_extension_payment_realex_remote.logger('Void result:\r\n' + JSON.stringify(void_response, 1));

			if ((void_response.result) && void_response.result == '00') {
				await this.model_extension_payment_realex_remote.addTransaction(realex_order['realex_remote_order_id'], 'void', 0.00);
				await this.model_extension_payment_realex_remote.updateVoidStatus(realex_order['realex_remote_order_id'], 1);
	...
	async capture() {
		...
		if ((this.request.post['order_id']) && this.request.post['order_id'] != '' && (this.request.post['amount']) && this.request.post['amount'] > 0) {
			...
			const capture_response = await this.model_extension_payment_realex_remote.capture(this.request.post['order_id'], this.request.post['amount']);

			await this.model_extension_payment_realex_remote.logger('Settle result:\r\n' + JSON.stringify(capture_response, 1));

			if ((capture_response.result) && capture_response.result == '00') {
				await this.model_extension_payment_realex_remote.addTransaction(realex_order['realex_remote_order_id'], 'payment', this.request.post['amount']);
				const total_captured = await this.model_extension_payment_realex_remote.getTotalCaptured(realex_order['realex_remote_order_id']);
				let capture_status = 0;
				if (total_captured >= realex_order['total'] || realex_order['settle_type'] == 0) {
					await this.model_extension_payment_realex_remote.updateCaptureStatus(realex_order['realex_remote_order_id'], 1);
					capture_status = 1;
				...
				await this.model_extension_payment_realex_remote.updateForRebate(realex_order['realex_remote_order_id'], capture_response.pasref, capture_response.orderid);
	...
	async rebate() {
		...
		if ((this.request.post['order_id']) && this.request.post['order_id'] != '') {
			...
			const rebate_response = await this.model_extension_payment_realex_remote.rebate(this.request.post['order_id'], this.request.post['amount']);

			await this.model_extension_payment_realex_remote.logger('Rebate result:\r\n' + JSON.stringify(rebate_response, 1));

			if ((rebate_response.result) && rebate_response.result == '00') {
				await this.model_extension_payment_realex_remote.addTransaction(realex_order['realex_remote_order_id'], 'rebate', this.request.post['amount'] * -1);

				const total_rebated = await this.model_extension_payment_realex_remote.getTotalRebated(realex_order['realex_remote_order_id']);
				const total_captured = await this.model_extension_payment_realex_remote.getTotalCaptured(realex_order['realex_remote_order_id']);
				let rebate_status = 0;
				if (total_captured <= 0 && realex_order['capture_status'] == 1) {
					await this.model_extension_payment_realex_remote.updateRebateStatus(realex_order['realex_remote_order_id'], 1);
					rebate_status = 1;
	...
	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/payment/realex_remote')) {
