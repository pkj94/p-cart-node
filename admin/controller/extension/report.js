...
const fs= require('fs');
module.exports = class ReportController extends global['OpencartSystemEngineController'] {
	/**
	 * @return void
	 */
	async index() {
	...
	async getList() {
		...
		const results = await this.model_setting_extension.getPaths('%/admin/controller/report/%.js');
		...
		const extensions = await this.model_setting_extension.getExtensionsByType('report');
		...
		if (results) {
			for (let result of results) {
				...
				data['extensions'].push({
					...
					'install': await this.url.link('extension/report.install', 'user_token=' + this.session.data['user_token'] + '&extension=' + extension + '&code=' + code),
					'uninstall': await this.url.link('extension/report.uninstall', 'user_token=' + this.session.data['user_token'] + '&extension=' + extension + '&code=' + code),
					'installed': installed.includes(code),
					'edit': await this.url.link('extension/' + extension + '/report/' + code, 'user_token=' + this.session.data['user_token'])
				});
			}
		}
	...
	async install() {
		...
		if (!fs.existsSync(DIR_EXTENSION + extension + '/admin/controller/report/' + code + '.js')) {
		...
		if (!Object.keys(json).length) {
			...
			await this.model_user_user_group.addPermission(await this.user.getGroupId(), 'access', 'extension/' + extension + '/report/' + code);
			await this.model_user_user_group.addPermission(await this.user.getGroupId(), 'modify', 'extension/' + extension + '/report/' + code);
...
this.autoloader.register('OpencartAdminControllerExtension' +namespace, DIR_EXTENSION + extension + '/admin/controller/');
			...
			await this.load.controller('extension/' + extension + '/report/' + code + '.install');
	...
	async uninstall() {
		...
		if (!Object.keys(json).length) {
			...
			await this.load.controller('extension/' + this.request.get['extension'] + '/report/' + this.request.get['code'] + '.uninstall');
