module.exports = class ControllerExtensionModuleSagepayDirectCards extends Controller {
	...
	async index() {
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/module/sagepay_direct_cards', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/module/sagepay_direct_cards', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=module', true);

		if ((this.request.post['module_sagepay_direct_cards_status'])) {
		...
		this.response.setOutput(await this.load.view('extension/module/sagepay_direct_cards', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/module/sagepay_direct_cards')) {
