module.exports = class ControllerExtensionModuleLatest extends Controller {
	error = {};

	async index() {
		await this.load.language('extension/module/latest');
		...
		if ((this.request.server['method'] == 'POST') && await this.validate()) {
			if (!(this.request.get['module_id'])) {
				await this.model_setting_module.addModule('latest', this.request.post);
			} else {
		...
		if (!(this.request.get['module_id'])) {
			data['action'] = await this.url.link('extension/module/latest', 'user_token=' + this.session.data['user_token'], true);
		} else {
			data['action'] = await this.url.link('extension/module/latest', 'user_token=' + this.session.data['user_token'] + '&module_id=' + this.request.get['module_id'], true);
		}
		...
		this.response.setOutput(await this.load.view('extension/module/latest', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/module/latest')) {
