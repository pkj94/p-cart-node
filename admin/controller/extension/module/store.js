...
await this.session.save(this.session.data);
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/module/store', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/module/store', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=module', true);

		if ((this.request.post['module_store_admin'])) {
			data['module_store_admin'] = this.request.post['module_store_admin'];
		} else {
			data['module_store_admin'] = this.config.get('module_store_admin');
		}

		if ((this.request.post['module_store_status'])) {
		...
		this.response.setOutput(await this.load.view('extension/module/store', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/module/store')) {
