module.exports = class ControllerExtensionModuleSagepayServerCards extends Controller {
	...
	async index() {
		await this.load.language('extension/module/sagepay_server_cards');
		...
		if ((this.request.server['method'] == 'POST') && await this.validate()) {
			await this.model_setting_setting.editSetting('module_sagepay_server_cards', this.request.post);
		...
		data['breadcrumbs'].push({
			...
			'href' : await this.url.link('extension/module/sagepay_server_cards', 'user_token=' + this.session.data['user_token'], true)
		});

		data['action'] = await this.url.link('extension/module/sagepay_server_cards', 'user_token=' + this.session.data['user_token'], true);

		data['cancel'] = await this.url.link('marketplace/extension', 'user_token=' + this.session.data['user_token'] + '&type=module', true);

		if ((this.request.post['module_sagepay_server_cards_status'])) {
		...
		this.response.setOutput(await this.load.view('extension/module/sagepay_server_cards', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/module/sagepay_server_cards')) {
