module.exports = class ControllerExtensionModuleBanner extends Controller {
	error = {};
	...
	async index() {
		...
		if ((this.request.server['method'] == 'POST') && await this.validate()) {
			if (!(this.request.get['module_id'])) {
				await this.model_setting_module.addModule('banner', this.request.post);
			} else {
		...
		if (!(this.request.get['module_id'])) {
			data['action'] = await this.url.link('extension/module/banner', 'user_token=' + this.session.data['user_token'], true);
		} else {
			data['action'] = await this.url.link('extension/module/banner', 'user_token=' + this.session.data['user_token'] + '&module_id=' + this.request.get['module_id'], true);
		}
		...
		this.response.setOutput(await this.load.view('extension/module/banner', data));
	}

	async validate() {
		if (!await this.user.hasPermission('modify', 'extension/module/banner')) {
