...
module.exports = class AddressFormatController extends global['OpencartSystemEngineController'] {
	/**
	 * @return void
	 */
	async index() {
	...
	async getList() {
		...
		const address_format_total = await this.model_localisation_address_format.getTotalAddressFormats(filter_data);
	...
	async save() {
		...
		if (!Object.keys(json.error).length) {
			this.load.model('localisation/address_format', this);
			this.request.post['address_format_id'] = Number(this.request.post['address_format_id']);
	...
	async delete() {
		...
		for (let address_format_id of selected) {
			...
			const country_total = await this.model_localisation_country.getTotalCountriesByAddressFormatId(address_format_id);
