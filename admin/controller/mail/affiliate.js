module.exports = class ControllerMailAffiliate extends Controller {
	async approve(&route, &args, &output) {
		...
		customer_info = await this.model_customer_customer.getCustomer(args[0]);
		...
		if (customer_info) {
			...
			data['login'] = store_url + '?route=affiliate/login';
			...
			mail.setText(await this.load.view('mail/affiliate_approve', data));
			mail.send();
		}
	}
	...
	async deny(&route, &args, &output) {
		...
		if (customer_info) {
			...
			mail.setSender(store_name);
			mail.setSubject(subject);
			mail.setText(await this.load.view('mail/affiliate_deny', data));
			mail.send();
		}
	}
}