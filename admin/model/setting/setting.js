module.exports = class SettingModel extends Model {
    ...
    async editSetting(code, data, store_id = 0) {
        ...
        for (const key in data) {
            ...
            if (key.startsWith(code)) {
                ...
                await this.db.query(`INSERT INTO setting SET store_id = ${store_id}, code = '${this.db.escape(code)}', key = '${this.db.escape(key)}', value = '${this.db.escape(valueToStore)}', serialized = {serialized}`);
            }
        }
    }
    ...
    async editValue(code, key, value, store_id = 0) {
        const serialized = Array.isArray(value) ? 1 : 0;
        const valueToStore = serialized ? JSON.stringify(value) : value;
        await this.db.query(`UPDATE setting SET value = '${this.db.escape(valueToStore)}', serialized = {serialized} WHERE code = '${this.db.escape(code)}' AND key = '${this.db.escape(key)}' AND store_id = ${store_id}`);
    }
}
