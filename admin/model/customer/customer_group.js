module.exports = class ModelCustomerCustomerGroup extends Model {
	...
	async editCustomerGroup(customer_group_id, data) {
		await this.db.query("UPDATE " + DB_PREFIX + "customer_group SET approval = '" + data['approval'] + "', sort_order = '" + data['sort_order'] + "' WHERE customer_group_id = '" + customer_group_id + "'");
	...
	async getTotalCustomerGroups() {
		const query = await this.db.query("SELECT COUNT(*) AS total FROM " + DB_PREFIX + "customer_group");
