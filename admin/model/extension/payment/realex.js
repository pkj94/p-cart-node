...
module.exports = class ModelExtensionPaymentRealex extends Model {
	...
	async void(order_id) {
		...
		if ((realex_order)) {
			...
			let merchant_id = this.config.get('payment_realex_merchant_id');
			let secret = this.config.get('payment_realex_secret');
	...
	async rebate(order_id, amount) {
		...
		if ((realex_order) && realex_order['rebate_status'] != 1) {
			...
			if (realex_order['settle_type'] == 2) {
				...
				if (!(realex_order['pasref_previous'])) {
			...
			let rebate_hash = sha1(this.config.get('payment_realex_rebate_password'));
	...
	async getOrder(order_id) {
		...
		const qry = await this.db.query("SELECT * FROM `" + DB_PREFIX + "realex_order` WHERE `order_id` = '" + order_id + "' LIMIT 1");

		if (qry.num_rows) {
			const order = qry.row;
			order['transactions'] = await this.getTransactions(order['realex_order_id']);
	...
	async getTransactions(realex_order_id) {
		const qry = await this.db.query("SELECT * FROM `" + DB_PREFIX + "realex_order_transaction` WHERE `realex_order_id` = '" + realex_order_id + "'");
