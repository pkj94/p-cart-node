...
class ModelExtensionPaymentEway extends Model {
	...
	async getOrder(order_id) {
		qry = await this.db.query("SELECT * FROM `" + DB_PREFIX + "eway_order` WHERE `order_id` = '" + order_id + "' LIMIT 1");
	...
	async capture(order_id, capture_amount, currency) {
		...
		if (eway_order && capture_amount > 0 ) {
			...
			capture_data.Payment.TotalAmount = (number_format(capture_amount, 2, '.', '') * 100);
	...
	async refund(order_id, refund_amount) {
		...
		if (eway_order && refund_amount > 0) {
			...
			refund_data.Refund.TotalAmount = (number_format(refund_amount, 2, '.', '') * 100);
	...
	private function getTransactions(eway_order_id) {
		qry = await this.db.query("SELECT * FROM `" + DB_PREFIX + "eway_transactions` WHERE `eway_order_id` = '" + eway_order_id + "'");
