module.exports = class ModelExtensionPaymentFirstdata extends Model {
	...
	async uninstall() {
		await this.db.query("DROP TABLE IF EXISTS `" + DB_PREFIX + "firstdata_order`;");
		await this.db.query("DROP TABLE IF EXISTS `" + DB_PREFIX + "firstdata_order_transaction`;");
		await this.db.query("DROP TABLE IF EXISTS `" + DB_PREFIX + "firstdata_card`;");
	}
	...
	async updateVoidStatus(firstdata_order_id, status) {
		await this.db.query("UPDATE `" + DB_PREFIX + "firstdata_order` SET `void_status` = '" + status + "' WHERE `firstdata_order_id` = '" + firstdata_order_id + "'");
	}
	...
	async updateCaptureStatus(firstdata_order_id, status) {
		await this.db.query("UPDATE `" + DB_PREFIX + "firstdata_order` SET `capture_status` = '" + status + "' WHERE `firstdata_order_id` = '" + firstdata_order_id + "'");
	}
	...
	async addTransaction(firstdata_order_id, type, total) {
		await this.db.query("INSERT INTO `" + DB_PREFIX + "firstdata_order_transaction` SET `firstdata_order_id` = '" + firstdata_order_id + "', `date_added` = now(), `type` = '" + this.db.escape(type) + "', `amount` = '" + total + "'");
	}
	...
	async getTotalCaptured(firstdata_order_id) {
		const query = await this.db.query("SELECT SUM(`amount`) AS `total` FROM `" + DB_PREFIX + "firstdata_order_transaction` WHERE `firstdata_order_id` = '" + firstdata_order_id + "' AND (`type` = 'payment' OR `type` = 'refund')");
