module.exports = class ModelExtensionPaymentSquareup extends Model {
    constructor(registry) {
        super(registry);
        this.RECURRING_ACTIVE = 1;
        this.RECURRING_INACTIVE = 2;
        this.RECURRING_CANCELLED = 3;
        this.RECURRING_SUSPENDED = 4;
        this.RECURRING_EXPIRED = 5;
        this.RECURRING_PENDING = 6;
    }
    ...
    async getTransactions(data) {
        let sql = "SELECT * FROM `" + DB_PREFIX + "squareup_transaction`";
    ...
    async getTotalTransactions(data) {
        let sql = "SELECT COUNT(*) of total FROM `" + DB_PREFIX + "squareup_transaction`";
    ...
    async getOrderStatusId(order_id, transaction_status = null) {
        if (transaction_status) {
            return this.config.get('payment_squareup_status_' + transaction_status.toLowerCase());
        } else {
