...
module.exports = class ModelExtensionDashboardSale extends Model {
	async getTotalSales(data = {}) {
		...
		return (query.row['total']) ? query.row['total'] : 0.00;
	}

	async getTotalOrdersByCountry() {
		const query = await this.db.query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" + DB_PREFIX + "order` o LEFT JOIN `" + DB_PREFIX + "country` c ON (o.payment_country_id = c.country_id) WHERE o.order_status_id > '0' GROUP BY o.payment_country_id");
