...
class OrderModel  extends Model
{
	...
	{
		order_query = await this.db.query("SELECT *, (SELECT `os`.`name` FROM `" + DB_PREFIX + "order_status` os WHERE `os`.`order_status_id` = `o`.`order_status_id` AND `os`.`language_id` = '" +  this.config.get('config_language_id') + "') AS `order_status` FROM `" + DB_PREFIX + "order` `o` WHERE `o`.`order_id` = '" +  order_id + "'");
		...
		if (order_query.num_rows) {
			...
			let country_query = await this.db.query("SELECT * FROM `" + DB_PREFIX + "country` WHERE `country_id` = '" +  order_query.row['shipping_country_id'] + "'");
			...
			let zone_query = await this.db.query("SELECT * FROM `" + DB_PREFIX + "zone` WHERE `zone_id` = '" +  order_query.row['shipping_zone_id'] + "'");
			...
			order_product_query = await this.db.query("SELECT * FROM `" + DB_PREFIX + "order_product` WHERE `order_id` = '" +  order_id + "'");
	 ...
	 * @return float
	 */
	async getTotalSales(data = {}): float
	{
	...
	async getEmailsByProductsOrdered(products, start, end)
	{
		...
		let query = await this.db.query("SELECT DISTINCT o.`email` FROM `" + DB_PREFIX + "order` o LEFT JOIN `" + DB_PREFIX + "order_product` op ON (o.`order_id` = op.`order_id`) WHERE (" + implode(" OR ", implode) + ") AND o.`order_status_id` <> '0' LIMIT " +  start + "," +  end);
	...
	async getTotalEmailsByProductsOrdered(products)
	{
		...
		let query = await this.db.query("SELECT COUNT(DISTINCT o.`email`) AS `total` FROM `" + DB_PREFIX + "order` o LEFT JOIN `" + DB_PREFIX + "order_product` op ON (o.`order_id` = op.`order_id`) WHERE (" + implode(" OR ", implode) + ") AND o.`order_status_id` <> '0'");
